...
switch($job){
    ...
    case 'salvar':{
        ...
        $ds_documento = $_REQUEST['ds_documento'];
        $frota_checklist_documentos = $frota_checklist_documentosdao->carregarPorPk($pk);
        if($ds_documento != "")
            $arrDsArquivos = json_decode ($ds_documento, true);
    ...
    case 'renomearArquivo':{
        ...
        $resultado = "";
        $query = $frota_checklist_documentosdao->listarQuantidadeDocumentosFrotaChecklist($frota_checklist_pk);
        
        $result  = 'success';
        $message = 'query success';

        if($frota_checklist_pk!=""){
            rename("../docs/$ds_documento", "../docs/".$frota_checklist_pk."Frota-".($query[0]['total']+1)."-".$ds_documento);
            $mysql_data[] = array(
                "t_ds_nome_salvo" => $frota_checklist_pk."Frota-".($query[0]['total']+1)."-".$ds_documento,
                "t_functions" => ""
            );
        }
        else{
            rename("../docs/$ds_documento", "../docs/Frota-".($query[0]['total']+1)."-".$ds_documento);
            $mysql_data[] = array(
                "t_ds_nome_salvo" => "Frota-".($query[0]['total']+1)."-".$ds_documento,
                "t_functions" => ""
            );
        }
