...
class financeiro_import_lancamento_itensdao{
    ...
    public function salvar($financeiro_import_lancamento_itens){
        
        $fields = array();
        $fields['ic_tipo_transacao'] = $financeiro_import_lancamento_itens['ic_tipo_transacao'];
        $fields['dt_transacao'] = DataYMD($financeiro_import_lancamento_itens['dt_transacao']);
    ...
    public function excluir($financeiro_import_lancamento_itens){
        $this->db->execDelete("financeiro_import_lancamento_itens"," pk = ".$financeiro_import_lancamento_itens->getpk());
    }

    public function carregarPorPk($pk){

        $financeiro_import_lancamento_itens = new financeiro_import_lancamento_itens();
        if($pk != ""){
        ...
        $sql.="  from financeiro_import_lancamento_itens ";
        $sql.=" where pk = $pk ";
            $query = $this->db->execQuery($sql);
            for($i = 0; $i < count($query); $i++){
                $financeiro_import_lancamento_itens->setpk($query[$i]["pk"]);
                $financeiro_import_lancamento_itens->setdt_cadastro($query[$i]["dt_cadastro"]);
                $financeiro_import_lancamento_itens->setusuario_cadastro_pk($query[$i]["usuario_cadastro_pk"]);
                $financeiro_import_lancamento_itens->setdt_ult_atualizacao($query[$i]["dt_ult_atualizacao"]);
                $financeiro_import_lancamento_itens->setusuario_ult_atualizacao_pk($query[$i]["usuario_ult_atualizacao_pk"]);

                $financeiro_import_lancamento_itens->setic_tipo_transacao($query[$i]['ic_tipo_transacao']);
                $financeiro_import_lancamento_itens->setdt_transacao($query[$i]['dt_transacao']);
                $financeiro_import_lancamento_itens->setvl_transacao($query[$i]['vl_transacao']);
                $financeiro_import_lancamento_itens->setcod_verificacao_transacao($query[$i]['cod_verificacao_transacao']);
                $financeiro_import_lancamento_itens->setds_estabelecimento($query[$i]['ds_estabelecimento']);
                $financeiro_import_lancamento_itens->setfinanceiro_conciliacao_banco_pk($query[$i]['financeiro_conciliacao_banco_pk']);

            }
        }
        return $financeiro_import_lancamento_itens;
    }
