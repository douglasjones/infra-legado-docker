...
function fcEnviarAgenda(){
   ...
   // NewWindow(v_last_url)
    ...
    if (arrEnviar.result == 'success'){
        ...
        if(ic_abertura == 1){
            sendPost("agenda_calendario_form.php", {token: token});
        }else if(ic_abertura == 2){
            sendPost("comercial_painel_res_form.php", {token: token});
        }else if(ic_abertura == 3){
            sendPost("lead_main_form.php", {token: token, leads_pk: leads_pk});
        } 
    }
    else{
...
function fcGridAgendaParticipantes(){   
    try {
        ...
        var v_url = montarUrlController("agendas_participantes", "listarDataTable", objParametros);
        ...
        tblParticipantesAgenda = $('#tblParticipantesAgenda').DataTable({
            ...
            "columnDefs": [{
                    "targets": -1,
                    "data": null,
                    "defaultContent":  "<i class='bi bi-x-circle function_delete' style='font-size:18px; color:black' title='EXCLUIR O PARTICIPANTE'></i>"
                },
                {"targets": -2, "data": "t_ds_email"},
                {"targets": -3, "data": "t_ds_cel"},
                {"targets": -4, "data": "t_ds_participante"},
                {"targets": -5, "data": "t_participante_pk", visible: false},
                {"targets": -6, "data": "t_ic_tipo_participante", visible: false},
                {"targets": -7, "data": "t_pk"}
            ],
            "language":{
...
function fcIncluirParticipante(){  
    ...
    tblParticipantesAgenda.row.add({
        ...
        "t_function":"<i class='bi bi-x-circle function_delete' style='font-size:18px; color:black' title='EXCLUIR O PARTICIPANTE'></i>",
    }).draw(false);   
...
try {
    ...
    /*if(data.length==0){
        alert('Inclua ao menos um Participante para salvar a agenda!'); 
        return false; 
    }*/
...
function fsClean() {
    $('#progressAgenda .progress-bar').css('width', '0%');
}
...
$(function () {
    $('#fileuploadAgenda').fileupload({
        ...
        progressall: function (e, data) {
            ...
            $('#progressAgenda .progress-bar').css('width', progress + '%');
        }
    });  
});
...
function fcFormatarDadosArquivosAgenda(){
    ...
    var arrDados = [];
        ...
        var colunas = $(this).children();
            DocAgendaPk =  $(colunas[0]).text(); 
            dsDocumento =  $(colunas[1]).text(); 
            dsNomeOriginal = $(colunas[2]).text();
            
            
            arrDados[i] = [DocAgendaPk,dsDocumento, dsNomeOriginal];
            i++;
        });
...
function fcAlterarNomeArquivoAgenda(v_arquivo){  
    ...
    if (arrEnviar.result == 'success'){
        // Reload datable
        $("#ds_documento_agenda").text(arrEnviar.data[0]['t_ds_nome_salvo']);
...
function fcCarregarGridDocumentosAgenda(){
    ...
    var v_url = montarUrlController("documento", "listarDocumentosAgenda", objParametros);
    //Trata a tabela
    tblDocumentosAgenda = $('#tblDocumentosAgenda').DataTable({
        ...
        "columnDefs": [{
                "targets": -1,
                "data": null,
                "defaultContent": "<i class='bi bi-cloud-arrow-up function_download' style='font-size:18px; color:black' title='DOWNLOAD DOCUMENTO'></i>&nbsp;&nbsp;&nbsp;&nbsp;<i class='bi bi-x-circle function_delete' style='font-size:18px; color:black' title='EXCLUIR O DOCUMENTO'></i>"
            },
    ...
    $('#tblDocumentosAgenda tbody').on('click', '.function_download', function () {
        ...
        fcDownloadDocumentoAgenda(data['t_ds_documento']);
    ...
    $('#tblDocumentosAgenda tbody').on('click', '.function_delete', function () {
        ...
        if(data['t_pk'] != ""){
            fcExcluirDocumentoAgenda(data['t_pk'],data['t_ds_documento']);
        }
    });
}

function fcDownloadDocumentoAgenda(ds_documento){
...
function fcExcluirDocumentoAgenda(v_pk,v_ds_documento){
