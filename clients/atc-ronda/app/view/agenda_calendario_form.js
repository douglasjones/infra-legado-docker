
function fcPesquisarEventos() {
    /* try {
         
         $('#calendario').clear().destroy();  
         runCalendar();
     } catch (error) {
         alert(error)
     }*/
 }
 
 function runCalendar() {
     try {
  
     var objParametros = {
         "leads_pk": $("#leads_pk_pesquisa").val()
     };    
     var v_url = montarUrlController("agenda_calendario", "listarEventos", objParametros);
     var calendar = $('#calendario').fullCalendar({
         
         slotDuration: '00:15:00', /* If we want to split day time each 15minutes */
         minTime: '00:00:00',
         maxTime: '23:59:59', 
         lang: 'pt-br',         
         header: {
             left: 'today',
             center: 'prevYear, prev, title, next, nextYear',
             right: 'month,agendaWeek,agendaDay'
         },
        // editable: true,
         selectable: true,
         events: {
             url: v_url,
         },
         eventRender: function(event, element) {
             if(event.icon){          
                 element.find(".fc-title").prepend("<i class='"+event.icon+"'></i> ");
             }
         },  
         eventClick: function (calEvent) {
             fcAbrirFormAgenda(calEvent.id, "", "edit");
         },
         select: function (startDate) {
             fcAbrirFormAgenda("", startDate.format(), "add");
         }
 
     });
     } catch (error) {
         alert(error)
     }
     
 }
 
 function fcListarUsuarioLogado(){
     var arrCarregar = carregarController("usuario", "listarUsuarioLogado", "");
     $("#usuario").html("<b>Olá "+arrCarregar.data[0]['ds_usuario']+"</b>")
 }
 
 function fcCarregarEquipesPesquisaAgenda(){
     
     var objParametros = {
         "pk": ""
     };
 
     var arrCarregar = carregarController("equipe", "listarTodos", objParametros);
     carregarComboAjax($("#equipes_pk_pesquisa"), arrCarregar, " ", "pk", "ds_equipe");
 }
 
 function fcCarregarLeadsPesquisaAgenda(){
     
     var objParametros = {
         "pk": ""
     };
 
     var arrCarregar = carregarController("lead", "listarLeadsCombo", objParametros);
     carregarComboAjax($("#leads_pk_pesquisa"), arrCarregar, " ", "leads_pk", "ds_lead");
 }
 
 
 $(document).ready(function() {
     runCalendar();
     fcListarUsuarioLogado()
     fcCarregarLeadsPesquisaAgenda();
     fcCarregarEquipesPesquisaAgenda();
 
     $("#cmdPesquisarAgenda").click(function(){
         fcPesquisarEventos();
    });
 }); 