...
function fcCarregarGrid(){
    ...
    var objParametros = {
        ...
        "leads_pk": leads_pk,
        "dt_escala_ini": dt_ini,
        "dt_escala_fim": dt_fim
    };         

    var arrCarregar = carregarController("colaborador_falta", "RelatorioPostoTrabalhoXColaboradorReserva", objParametros); 
    alert(v_last_url)
    if (arrCarregar.result == 'success'){
        ...
        if(arrCarregar.data.length > 0){   
                    ...
                    strRetorno+="<th width='20%'>Colaborador Fixo</th>";
                    strRetorno+="<th width='20%'>Colaborador Reserva</th>";
                    strRetorno+="<th width='20%'>Função</th>";
                    strRetorno+="<th width='20%'>Data Escala Reserva</th>";
                    strRetorno+="</tr>";
                    ...
                    for(i=0; i < arrCarregar.data.length ;i++){
                        ...
                        if(arrCarregar.data[i]['ds_colaborador_reserva']==null){
                            var ds_colaborador_reserva = "";
                        }
                        else{
                            var ds_colaborador_reserva = arrCarregar.data[i]['ds_colaborador_reserva'];
                        }

                        strRetorno+="<tr>";
                        strRetorno+="<th width='20%'>"+ds_lead+"</th>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['ds_colaborador']+"</th>";
                        strRetorno+="<th width='20%'>"+ds_colaborador_reserva+"</th>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['ds_produto_servico']+"</th>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['dt_escala']+"</th>";
                        strRetorno+="</tr>";
                }
                strRetorno+="</tbody>";
                strRetorno+="</table>";
...
function fcCancelar(){
    ...
    sendPost("rel_posto_trabalho_x_colaborador_reserva_res_form.php", {token: token});
}
