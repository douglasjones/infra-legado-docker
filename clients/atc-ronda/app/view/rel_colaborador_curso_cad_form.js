...
function fcCarregarGrid(){
    ...
    var arrCarregar = carregarController("colaborador_curso", "RelatorioColaboradorCurso", objParametros); 
    ...
    if (arrCarregar.result == 'success'){
        ...
        if(arrCarregar.data.length > 0){   
                    ...
                    strRetorno+="<tbody>";
                    strRetorno+="<tr>";
                    strRetorno+="<th width='20%'>Curso</th>";
                    strRetorno+="<th width='20%'>Colaborador</th>";
                    strRetorno+="<th width='20%'>Data Execução</th>";
                    strRetorno+="<th width='20%'>Data Vencimento</th>";
                    strRetorno+="</tr>";
                    
                    for(i=0; i < arrCarregar.data.length ;i++){
                        if(arrCarregar.data[i]['dt_execucao']==null){
                            var dt_execucao = "";
                        }
                        else{
                            var dt_execucao = arrCarregar.data[i]['dt_execucao'];
                        }
                        if(arrCarregar.data[i]['dt_validacao']==null){
                            var dt_validacao = "";
                        }
                        else{
                            var dt_validacao = arrCarregar.data[i]['dt_validacao'];
                        }

                        strRetorno+="<tr>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['ds_curso']+"</th>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['ds_colaborador']+"</th>";
                        strRetorno+="<th width='20%'>"+dt_execucao+"</th>";
                        strRetorno+="<th width='20%'>"+dt_validacao+"</th>";
                        strRetorno+="</tr>";
                }
                strRetorno+="</tbody>";
                strRetorno+="</table>";
...
function fcCancelar(){
    ...
    sendPost("rel_colaborador_curso_res_form.php", {token: token});
}
