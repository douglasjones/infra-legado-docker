...
function fcCarregarGrid(){
    ...
    var arrCarregar = carregarController("agenda_colaborador_pausa", "RelatorioColaboradorFerias", objParametros); 
    ...
    if (arrCarregar.result == 'success'){
        ...
        if(arrCarregar.data.length > 0){   
                    ...
                    strRetorno+="<th width='20%'>Data Inicio Férias</th>";
                    strRetorno+="<th width='20%'>Data Fim Férias</th>";
                    strRetorno+="</tr>";
                    
                    for(i=0; i < arrCarregar.data.length ;i++){
                        if(arrCarregar.data[i]['t_dt_inicio_pausa']==null){
                            var dt_inicio = "";
                        }
                        else{
                            var dt_inicio = arrCarregar.data[i]['t_dt_inicio_pausa'];
                        }
                        if(arrCarregar.data[i]['t_dt_fim_pausa']==null){
                            var dt_final = "";
                        }
                        else{
                            var dt_final = arrCarregar.data[i]['t_dt_fim_pausa'];
                        }

                        strRetorno+="<tr>";
                        strRetorno+="<th width='20%'>"+arrCarregar.data[i]['t_ds_colaborador']+"</th>";
                        strRetorno+="<th width='20%'>"+dt_inicio+"</th>";
                        strRetorno+="<th width='20%'>"+dt_final+"</th>";
                        strRetorno+="</tr>";
                }
                strRetorno+="</tbody>";
                strRetorno+="</table>";
...
function fcCancelar(){
    ...
    sendPost("rel_colaborador_ferias_res_form.php", {token: token});
}
...
$(document).ready(function(){    
    ...
    $("#dt_ferias").text(dt_ini+" até "+dt_fim);
