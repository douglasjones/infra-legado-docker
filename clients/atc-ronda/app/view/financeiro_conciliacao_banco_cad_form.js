...
function fcEnviar(){
    ...
    if(qtdeArquivo==0){
        alert("Por favor, inserir anexo OFX.");
        return false;
    }
    ...
    var arrEnviar = carregarControllerNew("financeiro_conciliacao_banco", "salvar", formdata);           

    if (arrEnviar.result == 'success'){
        // Reload datable
        alert(arrEnviar.message);
        sendPost("financeiro_conciliacao_banco_res_form.php", {token: token});
    }
    else{
...
function fcCarregar(){
    ...
    if(pk > 0){
        ...
        var arrCarregar = carregarController("financeiro_conciliacao_banco", "listarPk", objParametros);
...
function fcCarregarDatatable(){
    ...
    var v_url = montarUrlController("financeiro_import_conciliacao_itens", "listarDataTable", objParametros);
    //Trata a tabela
        ...
        "columnDefs": [
            {
                "targets": -1,
                "data": null,
                "defaultContent": "<a class='function_edit'><span><i class='bi bi-pencil-square' style='font-size:18px; color:black' title='EDITAR O LEAD'></i></span></a>"
            },
           {"targets": -2, "data": "t_ds_estabelecimento"},
           {"targets": -3, "data": "t_lancamentos_pk"},
           {"targets": -4, "data": "t_vl_transacao"},
           {"targets": -5, "data": "t_ds_transacao"},
           {"targets": -6, "data": "t_dt_transacao"},
           {"targets": -7, "data": "t_pk"},
           {"targets": -8, "data": null,
           "defaultContent": "<a class='function_selecionar'><input type='checkbox' name='conciliacao[]' ></input></a>"}
         ],
...
function fcAbrirModal(objParametros){
    ...
    if(objParametros['t_ic_status']==null){
        $("#ic_status_modal").val(2);
    }
    else{
        $("#ic_status_modal").val(objParametros['t_ic_status']);
    }
    ...
    $("#obs_modal").val(objParametros['t_obs']);
...
function fcCarregarDatatableReceitaDespesas(){
    ...
    var v_url = montarUrlController("lancamento", "listarDataTableReceitaDespesaConciliacao", objParametros);
    //Trata a tabela
    tblResultadoReceitaDespesa = $('#tblResultadoReceitaDespesa').DataTable({
        ...
        "pageLength":10,
        "columnDefs": [
           {"targets": -1, "data": "ds_recebido_pago_origem"},
           {"targets": -2, "data": "financeiro_conciliacao_banco_itens_pk"},
           {"targets": -3, "data": "vl_lancamento"},
           {"targets": -4, "data": "ds_operacao"},
           {"targets": -5, "data": "dt_vencimento"},
           {"targets": -6, "data": "pk"},
           {"targets": -7, "data": null,
           "defaultContent": "<a class='funcion_selecionar_lancamento'><input type='checkbox' name='lancamentoReceitaDespesa[]' disabled ></input></a>"}
...
function fcSalvarConciliacaoLancamento(){
    ...
    var arrEnviar = carregarController("financeiro_conciliacao_lancamentos", "salvar", objParametros);           
