function fcLimparUsuarioGridLancamentoFormulario() {
...
function fcCarregarUsuarioComboEmpresasLancamento() {
    var objParametros = {
...
function fcCarregarUsuarioComboContasLancamento() {
...
function fcCarregarUsuarioComboUsuarios() {
...
function fcListarUsuarioItensGruposLancamento() {
...
function fcImprimirLancamentoUsuario(pk) {   
    sendPost('financeiro_impressao_lancamento_form.php', { token: token, pk: pk, ic_origin:2});
}

function fcExcluirLancamentoUsuario(v_pk, v_status) {
    ...
    if (confirm("Deseja realmente excluir o registro '" + v_status + "'?")) {
        if (v_pk != "") {
...
function fcCarregarUsuarioGridLancamento() {
    ...
    var v_ds_num_documento_gridUsuario = $("#ds_num_documento_gridUsuario").val();
    var v_ic_tipo_num_documento_gridUsuario = $("#ic_tipo_num_documento_gridUsuario").val();
    ...
    var arrCarregar = carregarController("lancamento", "listarLancamentoGridUsuarios", objParametros);
    ...
    if (arrCarregar.result == 'success') {
        ...
        vhtml += "               <th width='5' align='center'><font style='font-size: 15px'><b>Identificação do Lançamento</b></font></th>";
        ...
        vhtml += "               <th align='center'><font style='font-size: 15px'><b>Status<br>Análise Financeira</b></font></th>";
        ...
        if (arrCarregar.data.length > 0) {
            ...
            for (i = 0; i < arrCarregar.data[0].DadosExtratoLancamento.length; i++) {
                ...
                if (arrCarregar.data[0].DadosExtratoLancamento[i].ds_lancamento != null) {
                    v_ds_lancamento = arrCarregar.data[0].DadosExtratoLancamento[i].ds_lancamento;
                }
                ...
                if (arrCarregar.data[0].DadosExtratoLancamento[i].ds_status_analise == null) {
                    ds_status_analise = "";
                }else{
                    ds_status_analise = arrCarregar.data[0].DadosExtratoLancamento[i].ds_status_analise;
                }
                ...
                vhtml += "               <td width='100' align='center'>" + v_ds_lancamento + "</td>";
                ...
                vhtml += "               <td align='center'>" + ds_status_analise + "</td>";
                vhtml += "               <td>";
                vhtml += "                  <table>";
                vhtml += "                      <tr>";
                vhtml += "                       <td>";
                if(ds_status_analise == "Aprovado Analista" || ds_status_analise == "Aprovado Gestor"){
                    vhtml += "                          <i style='font-size: 15px;' id='cmdEditar' class='bi bi-pencil-square' title='Editar Lançamento' onclick='alert(&apos;Lançamento Já foi Aprovado e não pode ser alterado!&apos;); return false;'></i>";
                }else if(ds_status_analise == "Recusado"){
                    vhtml += "                          <i style='font-size: 15px;' id='cmdEditar' class='bi bi-pencil-square' title='Editar Lançamento' onclick='alert(&apos;Lançamento Já foi Recusado e não pode ser alterado!&apos;); return false;'></i>";
                }else{
                    vhtml += "                          <i style='font-size: 15px;' id='cmdEditar' class='bi bi-pencil-square' title='Editar Lançamento' onclick='fcEditarLancamento(" + v_pk + ")' ></i>";
                }                   
                vhtml += "                       </td>";
                vhtml += "                       <td>";
                ...
                if (arrCarregarImpressao.result == 'success'){  
                    vhtml += "                       <td>";
                    vhtml += "                          <i style='font-size: 15px;' id='cmdImprimir' class='bi bi-printer' title='Imprimir Lançamento' onclick='fcImprimirLancamentoUsuario(" + v_pk + ")'></i>";
                    vhtml += "                       </td>";
                }
                vhtml += "                       <td>";
                vhtml += "                          <i style='font-size: 15px;' id='cmdExcluir' class='bi bi-x-circle' title='Excluir Lançamento' onclick='fcExcluirLancamentoUsuario(" + v_pk + "," + ic_status_pagamento + ")'></i>";
                vhtml += "                       </td>";
                vhtml += "                      </tr>";
               /* vhtml += "                      <tr>";
                vhtml += "                      </tr>";*/
			    if(ds_status_analise == "Correção Solicitada" || ds_status_analise == "Recusado"){
                    vhtml += "                      <tr>";
                    vhtml += "                      <td>";
                    vhtml += "                          <i style='font-size: 15px;' id='cmdObservacaoFinanceira' class='bi bi-card-text' title='Observação Financeira' onclick='fcObservacaoAnaliseFinanceira(&apos;"+v_pk+"&apos;, &apos;"+ds_status_analise+"&apos;)'></i>";
                    vhtml += "                      </td>";
                    vhtml += "                      </tr>";
                }
                vhtml += "                      </tr>";
...
function fcPesquisarUsuarioLancamento() {
    if($("#ic_status_pagamento_lancamento").val() == ""){
        if($("#ds_num_documento_gridUsuario").val() == ""){
            if($("#ic_tipo_num_documento_gridUsuario").val() == ""){
                if($("#empresa_lancamento_pk").val() == ""){
                    if($("#contas_lancamento_pk").val() == ""){
                        if($("#tipo_grupo_lancamento_pk").val() == ""){
                            if($("#ic_status_analise").val() == ""){
...
$(document).ready(function () {
    fcLimparUsuarioGridLancamentoFormulario();
    fcCarregarUsuarioComboEmpresasLancamento();
    fcCarregarUsuarioComboUsuarios();
    $(".chzn-select").chosen({ allow_single_deselect: true });

    $("#empresa_lancamento_pk").change(function () {
        $(".chzn-select").chosen('destroy');
        fcCarregarUsuarioComboContasLancamento();
        $(".chzn-select").chosen({ allow_single_deselect: true });
    });

    $("#tipo_grupo_lancamento_pk").change(function () {
        $(".chzn-select").chosen('destroy');
        fcListarUsuarioItensGruposLancamento();
        $(".chzn-select").chosen({ allow_single_deselect: true });
    });
    ...
    $(document).on('click', '#cmdPesquisaUsuarioGrid', function () {
    
        fcPesquisarUsuarioLancamento();
    });
