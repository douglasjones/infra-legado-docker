...
/*function fcMontarArrayItens(){
    try {
        ...
        for(var i=0; i<arrCarregar.data.length; i++){
            ...
            var propostas_facilities_grupos_subgrupos_pk = "";
            var ic_status = "";
                propostas_facilities_grupos_subgrupos_pk = arrCarregar.data[i]['propostas_facilities_grupos_subgrupos_pk']
                if(arrCarregar.data[i]['subgrupo_pk'] != null){
                    propostas_facilities_grupos_subgrupos_pk = arrCarregar.data[i]['subgrupo_pk']
                }
                
                propostas_facilities_label_pk = arrCarregar.data[i]['pk']
                ds_valor = $(".valor"+propostas_facilities_label_pk).val();
                ds_percentual = $(".percentual"+propostas_facilities_label_pk).val();
                itens_pk = $(".pk"+propostas_facilities_label_pk).val();
                ic_status = arrCarregar.data[i]['ic_status'] 

            arrDados[i] = [itens_pk, propostas_facilities_grupos_subgrupos_pk, propostas_facilities_label_pk, ds_valor, ds_percentual, ic_status];
...
function fcSalvarProposta(){
    ...
    var v_vl_salario_piso_categoria = $("#vl_salario_piso_categoria").val();
    var v_vl_total_proposta = $("#vl_total_proposta").val();
    var v_vl_total_percentual_proposta = $("#vl_total_percentual_proposta").val();
    ...
    if (arrEnviar.result == 'success'){
        // Reload datable
        salvarItensProposta(arrEnviar.data[0]['pk']);
        alert(arrEnviar.message);
        if(ic_abertura == 1){
            sendPost('proposta_facilities_res_form.php', {token: token});
        }else{
...
function fcCarregarDadosProposta(){
    if(pk>0){
        ...
        $("#produtos_servicos_pk").val(arrCarregar.data[0]['produtos_servicos_pk'])
