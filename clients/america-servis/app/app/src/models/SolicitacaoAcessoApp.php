...
class SolicitacaoAcessoApp {
    ...
    private function respostaApiValida($data){
        if (empty($data)) {
            return false;
        }
        ...
        if (isset($data->result) && ($data->result === false || $data->result === "false")) {
            return false;
        }
    ...
    public function liberarAcesso($solicitacao_acesso_app){
        ...
        try{
            $apiPk = isset($solicitacao_acesso_app['api_pk']) && $solicitacao_acesso_app['api_pk'] !== ""
                ? $solicitacao_acesso_app['api_pk']
                : $solicitacao_acesso_app['pk'];
            ...
            if ($apiPk !== "") {
                ...
                if (!$this->respostaApiValida($data)) {
                    $retorno->message = isset($data->message) ? $data->message : 'Falha ao liberar acesso na API externa';
                    return $retorno;
                }
                ...
                if ($colaboradorPk !== "") {
                    $fields = array();
                    $fields['ic_status'] = $solicitacao_acesso_app['ic_status'];
                    $fields['dt_aprovacao'] = "sysdate()";
                    $fields['usuario_aprovacao_pk'] = $_SESSION['session_user']['par1'];
                    Util::execUpdate("ponto_solicitacao_liberacao_app", $fields, " colaborador_pk = ".$colaboradorPk, $this->pdo);
                }
    ...
    public function refazerNovoRegistro($solicitacao_acesso_app){
        $retorno = new \StdClass; //Estrutura de retorno para controller
        $retorno->status = false; //Retorno setado status como false
        $retorno->data = []; //Retorno data setado como vazio
        ...
        try{
            ...
            $host = $_SERVER['HTTP_HOST'];
            $currentUrl = $protocol . $host;
            $usuarioAprovacaoPk = isset($_SESSION['session_user']['par1']) ? $_SESSION['session_user']['par1'] : "";

            $body = [
                'ds_link' =>$currentUrl,
                'pk' =>$solicitacao_acesso_app['pk'],
                'ic_status' =>$solicitacao_acesso_app['ic_status'],
                'usuario_aprovacao_pk' =>$usuarioAprovacaoPk,
            ];

            $request = $this->client->request('POST','https://webservice.gepros6.com.br/work/action.php?action=refazerNovoRegistro', [
                'json'=>$body
            ]);
            ...
            if (!$this->respostaApiValida($data)) {
                $retorno->message = isset($data->message) ? $data->message : 'Falha ao refazer novo registro na API externa';
                return $retorno;
            }
            ...
            $fields['ic_status'] = $solicitacao_acesso_app['ic_status'];
            $fields['dt_aprovacao'] = "sysdate()";
            $fields['usuario_aprovacao_pk'] = $usuarioAprovacaoPk;

            if (isset($solicitacao_acesso_app['colaborador_pk']) && $solicitacao_acesso_app['colaborador_pk'] !== "") {
            ...
            $retorno->status = true;
            $retorno->message = 'Novo registro solicitado com sucesso';
            $retorno->data = $solicitacao_acesso_app['pk'];
        }
        catch(Throwable $e){
            $retorno->message = $e->getMessage();
    ...
    public function listarGrid($colaborador_pk,$ds_pin,$ds_re,$ic_status){
        try{
            ...
            $body = [
                ...
                'ic_status' =>$ic_status,
                'displayRange' =>intval($displayRange),
                'displayStart' =>intval($displayStart),
            ];
            ...
            $retorno->message = 'Dados carregados com sucesso';
            $retorno->data = $return;
            $retorno->iTotalDisplayRecords = $count;
            $retorno->iTotalRecords = $count;
