...
class Lead {
    ...
    public $pdo;
    private $hasDiaFaturamentoColumn = null;

    public function __construct($pdo) {
        $this->pdo = $pdo;
    }
    private function hasDiaFaturamentoColumn(){
        if ($this->hasDiaFaturamentoColumn !== null) {
            return $this->hasDiaFaturamentoColumn;
        }
        ...
        $this->hasDiaFaturamentoColumn = ((int) $stmt->fetchColumn()) > 0;
        return $this->hasDiaFaturamentoColumn;
    }

    private function diaFaturamentoSelect($alias = ''){
        if ($this->hasDiaFaturamentoColumn()) {
            if ($alias !== '') {
                return $alias . ".dia_faturamento";
            }

            return "dia_faturamento";
        }

        return "null dia_faturamento";
    }
    ...
    public function salvar($lead){
        ...
        if($this->hasDiaFaturamentoColumn() && isset($lead['dia_faturamento']) && !empty($lead['dia_faturamento'])){
    ...
    public function listar_por_ds_lead($ds_lead,$ic_cliente,$supervisores_pk,$responsavel_pk,$ds_lead_grid,$ic_tipo_lead,$leads_pai_pk, $leads_clientes_pk,$segmentos_pk){
        ...
        $sql.="       ,l.leads_pai_pk";
        $sql.="       ,".$this->diaFaturamentoSelect('l');
        $sql.="  from leads l ";
