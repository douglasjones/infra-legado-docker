...
class Colaborador {
    ...
    public function listarPk($pk){
        ...
        // ENRIQUECE RESULTADO COM ds_status VIA API
        foreach ($rows as &$row) {
            ...
            try {
                $request = $this->client->request('POST', 'https://webservice.gepros6.com.br/work/action.php?action=getImagemLiberacaoApp', [
                    'json' => $body
                ]);
                ...
                if ($code == 200) {
                    ...
                    $row['ds_imagem'] = isset($queryApi[0]->img_colaborador) ? $queryApi[0]->img_colaborador : null;
                    $row['dt_liberacao'] = isset($queryApi[0]->dt_liberacao) ? $queryApi[0]->dt_liberacao : " ";
                    $row['ds_status_app'] = isset($queryApi[0]->ic_status) ? $queryApi[0]->ic_status : " Pendente Cadastro ";
                    $row['liberar_acesso_ponto'] = isset($queryApi[0]->pk) ? $queryApi[0]->pk : "";
                } else {
                    $row['ds_imagem'] = null;
                    $row['dt_liberacao'] = null;
                    $row['ds_status_app'] = null;
                    $row['liberar_acesso_ponto'] = null;
                }
    ...
    public function salvarColaboradorServidor($pk){
        ...
        $sqlConta = "";
        $sqlConta .= "select c.ds_cpf_cnpj ";
        $sqlConta .= "  from contas c ";
        $sqlConta .= " where c.ic_status = 1 ";
        $sqlConta .= "   and c.pk = ".$colaborador['empresas_pk'];

        $stmtConta = $this->pdo->prepare($sqlConta);
        $stmtConta->execute();
        $contas = $stmtConta->fetchAll(\PDO::FETCH_ASSOC);
        ...
        $body = [
            ...
            'colaborador_pk'  => $pk,
            'ds_cnpj_conta'  => !empty($contas[0]['ds_cpf_cnpj']) ? $contas[0]['ds_cpf_cnpj'] : "",
            'ds_link' =>$currentUrl,
        ];
	        ...
	        try {
	            ...
	            error_log('Falha ao sincronizar colaborador no servidor externo: '.$e->getMessage());
	        }
	    }
