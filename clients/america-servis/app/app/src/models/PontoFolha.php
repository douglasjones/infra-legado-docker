...
class PontoFolha {
                ...
                //if($diasEscala[$i]['ic_escala'] == 1){
                    ...
                    if (!empty($arrNormal)) {
                        ...
                        if ($temEntrada && $temSaida) {
                            ...
                            // Mesmo incompleto, ainda é válido
                            $query = $arrNormal;
                        } elseif (count($arrNormal) >= 1) {
                            // Tem qualquer ponto (intervalo por exemplo)
                            $query = $arrNormal;
                        } else {
                            $query = $arrNoturno;
                        }
  ...
  public function pegarHorarioDeEntradaPorDataDiaSemana($colaboradores_pk, $dt_escala, $agenda_colaborador_padrao_pk = "")
    ...
    public function listarConsultaPontoColaborador($leads_pk, $colaboradores_pk, $dt_periodo_ini,$dt_periodo_fim,$agenda_colaborador_padrao_pk){              
        ...
        if (empty($dadosEscala)) {
            return []; // Retorna vazio se não encontrar escala
        }
    ...
    public function getPontoFolhaPk($leads_pk,$colaborador_pk,$ic_mes,$ic_ano){
        try{
            ...
            $retorno->data = []; //Retorno data setado como vazio
            $usuarioSessaoPk = 1;
            if (isset($_SESSION['session_user']['par1']) && $_SESSION['session_user']['par1'] !== "") {
                $usuarioSessaoPk = $_SESSION['session_user']['par1'];
            }
    ...
    public function gerarFolhaPontoByRelogio($arrDados){
        ...
        try{
            ...
            if (!is_array($arrDados) || count($arrDados) === 0) {
                throw new \Exception("Dados da folha não informados");
            }
            ...
            $dtFim = isset($arrDados[0]['dt_fim']) ? $arrDados[0]['dt_fim'] : "";
            $icMes = isset($arrDados[0]['ic_mes']) ? $arrDados[0]['ic_mes'] : "";
            $icAno = isset($arrDados[0]['ic_ano']) ? $arrDados[0]['ic_ano'] : "";

            if ($dtInicio !== "" && $dtFim !== "") {
                $primeiroDiaMesAtual = Util::DataYMD($dtInicio);
                $ultimoDiaMesAtual = Util::DataYMD($dtFim);
                $icMes = date("m", strtotime($primeiroDiaMesAtual));
                $icAno = date("Y", strtotime($primeiroDiaMesAtual));
            } else {
                $p1 = $icAno."-".$icMes."-01";
            ...
            //if($arrDados[0]['ic_status_ponto_folha_pk']!=1){
                ...
                $ponto_folha_pk = $this->getPontoFolhaPk($arrDados[0]['leads_pk'],$arrDados[0]['colaborador_pk'],$icMes,$icAno);
