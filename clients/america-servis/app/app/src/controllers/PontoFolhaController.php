...
final class PontoFolhaController extends BaseController {
    ...
    public function listarConsultaPontoColaborador(Request $request, Response $response, $args) {
        ...
        try{
            ...
            if(($dt_inicio == "" || $dt_inicio == "undefined" || $dt_fim == "" || $dt_fim == "undefined")
                && isset($data['ic_mes'], $data['ic_ano'])
                && $data['ic_mes'] != "" && $data['ic_mes'] != "undefined"
                && $data['ic_ano'] != "" && $data['ic_ano'] != "undefined"
            ){
                $ultimoDiaMes = cal_days_in_month(CAL_GREGORIAN, (int)$data['ic_mes'], (int)$data['ic_ano']);
                $dt_inicio = "01/".$data['ic_mes']."/".$data['ic_ano'];
                $dt_fim = $ultimoDiaMes."/".$data['ic_mes']."/".$data['ic_ano'];
            }
            ...
            $dt_periodo_ini  = Util::DataYMD($dt_inicio);
            $dt_periodo_fim  = Util::DataYMD($dt_fim);
