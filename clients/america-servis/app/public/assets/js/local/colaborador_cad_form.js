    ...
    if($("#colaborador_pk").val() > 0){
        ...
        if (arrCarregar.status == true){
            ...
            if(arrCarregar.data[0]['ds_imagem']!=null){
                ...
                var ds_tipo_imagem = ds_imagem.indexOf('/9j/') === 0 ? 'image/jpeg' : 'image/png';
                $("#ds_imagem").html("<img width='100' height='100' src='data:" + ds_tipo_imagem + ";base64,"+(ds_imagem)+"'>");
            }else{
...
function fcLiberarAcesso(e){
    ...
    utilsJS.jqueryConfirm('Liberar ?', 'A imagem esta dentro dos padrões para validação facial ? ', function () {
        ...
        if (arrEnviar && arrEnviar.status == true){
            ...
            utilsJS.toastNotify(false, arrEnviar && (arrEnviar.message || arrEnviar.result) ? (arrEnviar.message || arrEnviar.result) : "Falha ao liberar acesso");
        }

    });
    
}
function fcRefazerNovoRegistro(e){
    if (e) {
        e.preventDefault();
    }
    utilsJS.jqueryConfirm('Refazer ?', 'Ao confirmar o colaborador terá que repetir o processo de novo registro ', function () {
        ...
        if (arrEnviar && arrEnviar.status == true){
            ...
            utilsJS.toastNotify(false, arrEnviar && (arrEnviar.message || arrEnviar.result) ? (arrEnviar.message || arrEnviar.result) : "Falha ao solicitar novo registro");
        }
...
$(document).ready(function()
    {
        ...
        formdata = new FormData();
            ...
            console.error(err);
            utilsJS.toastNotify(false, 'Falha ao carregar parte dos dados do colaborador');
        } finally {
            $(".loader").hide();
            $("#carregar").hide();
            $("#exibir").show();
