...
$acao = 'cs';
if(!(($acao == 'cs' && permissao('rel_finul', 'cs')) || ($acao == 'upd' && permissao('rel_finul', 'al')))){
...
<table align="center">
	<tr>
		<td class="form"><font size="4">Relatório Funil de Vendas</font></td>
	</tr>
</table>
<table>
	<tr>
...
$where = "";
	...
	while($row = mysql_fetch_array($result)){
		echo "<tr>";
            ...
            echo "<td align='center' class='form'>&nbsp;".$row['dsc_operador']."&nbsp;</td>";
            echo "<td align='center' class='form'>&nbsp;".$row['dsc_tipo_linha']."&nbsp;</td>";
            echo "<td align='center' class='form'>&nbsp;".$row['cod_proposta']."&nbsp;</td>";
            echo "<td align='center' class='form'>&nbsp;".$row['dt_cad_proposta']."&nbsp;</td>";
            echo "<td align='center' class='form'>R$ ".number_format($row['vl_total_proposta'],2,",",".")."</td>";	
            
            $sql="";
            $sql.="Select
                     max(oc.CodOcorrenciaLead),
                     date_format(oc.dt_retorno,'%d/%m/%Y') dt_retorno
                    from ocorrenciaslead oc 
                    where  oc.CodLead=".$row['codlead']; 
            $sql.="  and oc.dt_retorno is not null 
                    and oc.CodUsuarioInterno=".$row['codgerenteconta'];
            //$sql.=" and oc.dt_retorno_fechamento is not null";
            if(!empty($dt_ini_oc)){
                $sql.=" and oc.dt_retorno >= '".DataYMD($dt_ini_oc)." 00:00:00' ";
            }
            if(!empty($dt_fim_oc)){
                $sql.=" and oc.dt_retorno <= '".DataYMD($dt_fim_oc)." 23:59:59' ";
            }
           
            $result_oc = mysql_query($sql);
            $row_oc = mysql_fetch_array($result_oc);
            if(!empty($row_oc['dt_retorno'])){
                echo "<td align='center' class='form'>&nbsp;".$row_oc['dt_retorno']."&nbsp;</td>";
            }else{
                echo "<td align='center' class='form'>&nbsp;</td>";
            }           
            mysql_free_result($result_oc);
            ...
            $sql.="Select
                max(oc.CodOcorrenciaLead),
                    ...
                    where  oc.CodLead=".$row['codlead']; 
            $sql.="  and oc.dt_retorno is not null 
                    and oc.agendadopara=".$row['codgerenteconta']; 
            $sql.=" and oc.CodUsuarioInterno=".$row['codgerenteconta'];
            $sql.=" and oc.dt_retorno_fechamento is null
                    and oc.dt_retorno >=sysdate()";     
            
     
            $results = mysql_query($sql);
            $rows = mysql_fetch_array($results);
            if(!empty($rows['dt_retorno'])){
                echo "<td align='center' class='form'>&nbsp;".$rows['dt_retorno']."&nbsp;</td>";
            }else{
                echo "<td align='center' class='form'>&nbsp;</td>";
            }
