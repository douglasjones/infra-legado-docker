...
<?
	...
	$sql = "SELECT l.CodLead,
			  ...
			  FROM leads l
				   ...
				   LEFT JOIN n_produto_tipo_linha nptl ON nipo.tipo_linha_pk = nptl.pk
                                   left join ocorrenciaslead oc on l.codlead = oc.codlead
                                   left join tipoocorrenciaslead toc on oc.codtipoocorrencialead = toc.codtipoocorrencialead
				   ...
				   INNER JOIN n_data_proposta_operador ndpo
					  ON     ndp.data_proposta_operador_pk = ndpo.pk
						 AND ndpo.ds_label_data = 'Recebimento do Pedido - 80%'";
	...
	if(!empty($_REQUEST['datarecebini']) and !empty($_REQUEST['datarecebfim'])){
		$sql .= " and ndp.vl_data_proposta >= '".DataYMD($_REQUEST['datarecebini']) . " 00:00:00'";	
		$sql .= " and ndp.vl_data_proposta <= '".DataYMD($_REQUEST['datarecebfim']) . " 23:59:59'";
	}	
	...
	if(!empty($_REQUEST['cod_polo'])){			
        ...
        if(!empty($_REQUEST['codtipoocorrencialead'])){
            $sql .= " and toc.codtipoocorrencialead = ".$_REQUEST['codtipoocorrencialead'];
        }
			...
			while($row80 = mysql_fetch_array($result80)){						  			
                ...
                $sql.= "SELECT nip.pk,
                        ...
                        FROM n_itens_propostas nip
                            ...
                            LEFT JOIN n_produto_tipo_linha nptl ON nipo.tipo_linha_pk = nptl.pk
                        WHERE nip.propostas_pk=".$row80['propostas_pk'];
                ...
                while($row = mysql_fetch_array($results)){
                    if($row['tipo_linha_pk']==1){ 
                         $novas = $row['n_qtde'];  
                    }
                    if($row['tipo_linha_pk']==2){ 
                         $adicao = $row['n_qtde'];  
                    } 
                    if($row['tipo_linha_pk']==3){ 
                         $renovacao = $row['n_qtde'];  
                    }
                    if($row['tipo_linha_pk']==4){ 
                         $portabilidade = $row['n_qtde'];  
                    }
                    if($row['tipo_linha_pk']==5){ 
                         $migracao = $row['n_qtde'];  
                    } 
                    if($row['tipo_linha_pk']==6){ 
                         $transferencia = $row['n_qtde'];  
                    } 
                    
                }
                $total_linhas += $novas + $adicao + $renovacao + $portabilidade + $migracao + $transferencia;
