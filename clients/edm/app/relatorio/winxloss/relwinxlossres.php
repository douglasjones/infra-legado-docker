	...
	<table cellspacing="0" cellpadding="0" align="center" border="1" width="90%" class="sortable">	
		<thead>
			<tr>
				...
				<td class="titulo" bgcolor="#8080FF">Qtde de Negócios</td>			
		...
		$sql ="";
		$sql.="select l.codlead, date_format(dp.valor_data, '%d/%m/%Y') recebe_assinatura, p.codproposta, p.versao, ml.descricao ";
		$sql.="  from leads l  ";
		...
		while($row_propostas = mysql_fetch_array($rs_propostas)){
			$arrQtdeNegocios[$row_propostas['descricao']] ++;
			...
			while($row_qtde_linhas = mysql_fetch_array($rs_qtde_linhas)){
				$arrQtdeLinhas[$row_propostas['descricao']] += $row_qtde_linhas['linhas'];
			}
			mysql_free_result($rs_qtde_linhas);
		}
		mysql_free_result($rs_propostas);
		
		//Exibe as informações no relatório
		foreach($arrMotivoSemInteresse as $valor){
			
			if($arrQtdeNegocios[$valor] > 0 && $arrQtdeLinhas[$valor] > 0){
			
				echo "<tr>";
				echo "<td class='form'>".$valor."</td>";
				echo "<td class='form' align='center'><a href='javascript: abrirAnalitico(".$arrMotivoSemInteresseCodigo[$valor].")'>".$arrQtdeNegocios[$valor]."</a></td>";
				echo "<td class='form' align='center'>".$arrQtdeLinhas[$valor]."</td>";
				echo "</tr>";
				
				//Calcula o total
				$total_negocios += $arrQtdeNegocios[$valor];
				$total_linhas += $arrQtdeLinhas[$valor];
