...
<br />  
	...
	if(!empty($ativacaode)){
		$sql.="   inner join data_proposta dp1 on (dp1.codlead = p.codlead 
                      and dp1.codproposta = p.codproposta
    	              and dp1.versao = p.versao 
    	              and dp1.nome_data = 'entrega_aparelho') ";
	}
    

		
	$sql.=" where np.dt_cancelamento IS NULL 
            And l.CodStatusClassificacaoLead >=10";
	...
	if(!empty($ativacaode)){
		$sql.=" and (dp1.valor_data between '".DataYMD($ativacaode)."' and '".DataYMD($ativacaoate)."' ) ";
	}
