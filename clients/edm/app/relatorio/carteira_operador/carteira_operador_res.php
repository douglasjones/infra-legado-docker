...
<table width="100%" align="center"  height="5"  cellpadding="0" cellspacing="0">
	<tr>
		<td>
			<font size="+2" face="Arial">Carteira Operador</font>
		</td>	
	</tr>
</table>
<br>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0">
	<tr>
		<th width="400" nowrap class='titulo' bgcolor='#8080FF'>Operador</th>
		<?
	...
	//traz a linha onde todos não existe consultor cadastrado
	//Pega todas as linhas sem gerente de conta
	if(permissao('visualizar_todos_atendentes', 'cs') && empty($codatendente)){
		...
		$sql.=" where l.codatendente is null ";
	...
	$sql = "";
	$sql.="select ui.nome atendente, ui.desativado, scl.descricao, count(*) total ";
	$sql.="  from leads l ";
	...
	//if(!permissao('visualizar_todos_consultor', 'cs')){
	...
	while($row = mysql_fetch_array($result)){
		...
		if($atendente != $row['atendente']){
			
			if($desativado == 1){
				echo "<td class='form'><font color='#990000'>$atendente</font></td>";
			}
			else{
				echo "<td class='form'><font color='#009900'>$atendente</font></td>";
			}
