...
class config_empresa {
	//REGISTRA DADOS DA EMPRESA MATRIZ OU FILIAL
	function dados_empresa($value){
		...
		if(!isset($value['origem_email_agendamento_pk'])) $value['origem_email_agendamento_pk'] = null;
		if(!isset($value['enviar_agenda_email_pk'])) $value['enviar_agenda_email_pk'] = null;
		if(!isset($value['agenda_email'])) $value['agenda_email'] = null;
		if(!isset($value['origem_email_proposta_pk'])) $value['origem_email_proposta_pk'] = null;
		if(!isset($value['enviar_proposta_email_pk'])) $value['enviar_proposta_email_pk'] = null;
		if(!isset($value['proposta_email'])) $value['proposta_email'] = null;
		...
		$fields['origem_email_agendamento_pk'] = $value['origem_email_agendamento_pk'];	
		$fields['enviar_agenda_email_pk'] = $value['enviar_agenda_email_pk'];	
		$fields['agenda_email'] = $value['agenda_email'];	
		$fields['origem_email_proposta_pk'] = $value['origem_email_proposta_pk'];	
		$fields['enviar_proposta_email_pk'] = $value['enviar_proposta_email_pk'];	
		$fields['proposta_email'] = $value['proposta_email'];	
		...
		//DADOS DE EMAIL
		if($value['config']==6){
			$sql = sqlinsert('email_empresa', array('cod_tipoemail'=> $value['cod_tipoemail'],'cod_empresa' => $value['cod_empresa'],'email_saida' => $value['email_saida'],'email_assunto' => $value['email_assunto'],'codtipoocorrencialead'=> $value['codtipoocorrencialead'],'codtipoocorrencialead'=>  $value['codtipoocorrencialead'],'email_texto'=>  $value['email_texto'],'dat_cad'=>"SYSDATE()"));				
			sql_query($sql);			
			$cod_empresa = $value['cod_empresa'];
		}	

		//RETORNA O CODIGO DA EMRPESA		
		return array('cod_empresa' => $cod_empresa, 'cod_operador' => $value['cod_operador']);
	}
}
?>
