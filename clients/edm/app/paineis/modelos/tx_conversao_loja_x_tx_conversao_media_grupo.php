...
$strGrafico.="<dataset seriesName='Tx Conversão Loja' color='1D8BD1' anchorBorderColor='1D8BD1' anchorBgColor='1D8BD1'> \n";
...
foreach($arrDatas as $data){
	...
	//Pega a quantidade de cupons da loja.
	$sql ="";
	$sql.="select ifnull(sum(n_qtde_transacoes),0) total_cupons ";
	$sql.="  from transacoes t ";
	$sql.=" where t.sites_pk = $sites_pk ";
	$sql.="   and t.dt_base = '".$data."' ";
	...
	if($total_cupons > 0)
		$strGrafico.= "<set value='".(($total_cupons / $total) * 100)."'/> \n";
	else
		$strGrafico.= "<set value='0'/> \n";	
}
$strGrafico.="</dataset> \n";
$strGrafico.="<dataset seriesName='Tx Conve. Média Grupo' color='#FFFF00' anchorBorderColor='#FFFF00' anchorBgColor='#FFFF00'> \n";
foreach($arrDatas as $data){
	...
	while($row = mysql_fetch_array($result)){
		...
		//Pega a quantidade de transacoes da loja
		$sql ="";
		$sql.="select ifnull(sum(n_qtde_transacoes),0) total_cupons ";
		$sql.="  from transacoes t ";
		$sql.=" where t.sites_pk = ".$row['pk'];
		$sql.="   and t.dt_base = '".$data."' ";
		$rs_cupons = mysql_query($sql);
		$row_cupons = mysql_fetch_array($rs_cupons);
		$total_cupons += $row_cupons['total_cupons'];
		if($total_cupons > 0){
			$total += $row['total'];
			$sql ="";
			$sql.="select n_qtde_desconto ";
