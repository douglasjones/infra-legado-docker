...
$strGrafico.= "<graph caption='' PYAxisName='Contagem' SYAxisName='Ticket Medio' numberPrefix='' showvalues='0' numDivLines='4' formatNumberScale='0' decimalPrecision='2' anchorSides='10' anchorRadius='3' anchorBorderColor='009900'  rotateNames='1' > \n";
...
foreach($arrDatas as $data){
	$sql ="";
	$sql.="select ifnull(s.n_qtde_desconto_diario,0) n_qtde_desconto_diario, ifnull(sum(co.enters),0) total ";
	$sql.="  from sites s ";
	...
	$sql.=" group by ifnull(s.n_qtde_desconto_diario,0) ";
	...
	if($total > 0)
		$strGrafico.= "<set color='".$arrCor[$indice]."' value='".$total."'/> \n";
	else
		$strGrafico.= "<set color='".$arrCor[$indice]."' value='0'/> \n";
