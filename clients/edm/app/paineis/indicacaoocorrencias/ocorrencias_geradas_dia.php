...
$sql.="		  inner join config_painel cp on ui.codusuariointerno = cp.codusuariointerno ";
...
for ($i = 0; $i < count($tipo_ocorrencia);$i++){
	...
	while($row = mysql_fetch_array($result)){
		...
		while($row_oper = mysql_fetch_array($result_oper)){
			...
			$sql = "";
			$sql.="select count(*) total from ocorrenciaslead where datacadastro between '$data' and '$data 23:59:59' and codusuariointerno = ".$row_oper['codusuariointerno']." and codtipoocorrencialead = ".$tipo_ocorrencia[$i];
...
$result = sql_query($sql);
while($row = mysql_fetch_array($result)){
	$sql="select count(*) total from ocorrenciaslead where datacadastro between '$data' and '$data 23:59:59' and codusuariointerno = ".$row['codusuariointerno']." and codtipoocorrencialead not in (";
	for($i = 0;$i<count($tipo_ocorrencia);$i++){
