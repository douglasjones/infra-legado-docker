...
for($i = 0; $i <= 6; $i++){
	//pega as ocorrencias
	$sql ="";
	$sql.="select date_format(vencimentocontrato, '%m') mes, count(*) total ";
	$sql.="  from leads l ";
	$sql.=" where (vencimentocontrato between DATE_FORMAT(DATE_ADD(SYSDATE(), INTERVAL $i MONTH),'%Y-%m-01 00:00:00') ";
	$sql.="                        and DATE_FORMAT(LAST_DAY(DATE_ADD(SYSDATE(), INTERVAL $i MONTH)),'%Y-%m-%d 23:59:59')) ";
	...
	$sql.=" group by date_format(vencimentocontrato, '%m') ";
