...
$strGrafico.="<graph caption='' xAxisName='' yAxisName='Valor Proposta(s)' decimalPrecision='0' formatNumberScale='0'>	 ";
//for($i = 12; $i >= 0; $i--){
	//pega as ocorrencias
$sql ="";
$sql.="SELECT 
        s.descricao,
        sum(np.vl_total_proposta) total,
...
$sql.=" order by s.CodStatusClassificacaoLead  ";
	...
	while($row = mysql_fetch_array($result)){
		$strGrafico.="<set name='".$row['descricao']."' value='".$row['total']."' color='".$arrCor[$row['codstatusclassificacaolead']]."'/> ";
