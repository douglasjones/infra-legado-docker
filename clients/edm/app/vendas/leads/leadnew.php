...
require "../../libs/funBuscarCep.php"; // Fun��o que faz a busca do cep
...
function buscaCep($cep, $endereco, $bairro, $cidade, $uf){
	...
	// Coloca os valores dos arrays nos campos do formul�rio
...
$ajax->printJavascript('../../libs/ajax/');
	...
	if(!empty($_REQUEST['excluir'])){
		...
		{
			javascriptalert('Opera��o executada com sucesso!!!' , false ); ?>
	...
	if(!empty($_REQUEST['enviar'])){
		...
		if(empty($_REQUEST['CodLead'])){
			if($codlead = leads::adicionar($_REQUEST)){
				javascriptalert('Opera��o executada com sucesso!!!', false, false);?>
	<script type="text/javascript" language="javascript">
		if(opener){
			if(opener.top.pagina)
...
<?			}else{
				...
				javascriptalert('Falha na opera��o!!!');
			}
		}else{
...
function salvarContato(e){
	...
	if(tel_contato_bloqueado.value == 1)
		r.insertCell(r.cells.length).innerHTML = "Bloqueado N�o Perturbe";
	else
...
Event.observe(window, 'load', function(e){
	...
	//Fun��o de Caixa de Sugestoes Leads
...
<?	if($Root){?>
						...
						&nbsp;�s&nbsp;<input type="text" id="horariocadastro" name="datacadastro[]" size="8" value="<?=(!empty($_REQUEST['DataCadastro'])?date('H:i:s', strtotime($_REQUEST['DataCadastro'])):null);?>" maxlength="8" validate="datatype=time" />
<?	}else{?>
						<?=(!empty($_REQUEST['DataCadastro'])?date('d/m/Y \�\s H:i:s', strtotime($_REQUEST['DataCadastro'])):null);?>
<?	} ?>					</td>
	  </tr>
				<tr>
				...
				<tr>
					<td>&nbsp;<label for="ie">Inscri��o Estadual:</label></td>
					<td>
						<input type="text" id="ie" name="ie" maxlength="50" size="22" value="<?=@$_REQUEST['IE'];?>" />					</td>
				</tr>
				<tr>
					<td>&nbsp;<label for="inscricaomunicipal">Inscri��o Municipal:</label></td>
					<td>
			   ...
			   <?
				...
				?>
				   <tr>
						<td>&nbsp;<label for="senhacliente">Identifica��o:</label></td>
						<td>
				...
				<tr>
					...
					<td>
						...
						<a href="javascript: consulta_np_telefone()" title="Consultar bloqueio N�o Perturbe - Procon/SP"><img border="0" src="../../images/interrogacao.png"></a>
						<font id="tel_cor" face="Arial" color="red"><span id="tel_descricao_bloqueado"><?
						if($_REQUEST['tel_bloqueado'] == 1)
							echo "Bloqueado N�o Perturbe";
						?></span></font>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<a href="javascript: consulta_np_fax()" title="Consultar bloqueio N�o Perturbe - Procon/SP"><img border="0" src="../../images/interrogacao.png"></a>
                ...
                <tr>
                 <td >&nbsp;<label for="cidade">Cidade:</label></td>
                  <td align="left"><input type="text" id="cidade" name="cidade" maxlength="29" size="29" value="<?=@$_REQUEST['cidade'];?>" />
					...
					<td>
						...
						if($GerenteContas){													
							...
							$row = mysql_fetch_array($result);
                                                        ...
                                                        $nome_consultor = $row['Nome'];
                                                        mysql_free_result($result);
                                                        
                                                        //verifica se o maiing com o nome do consultor ja esta cadastrado
                                                        $sql ="";
                                                        $sql.="Select
                                                                m.pk
                                                               from mailing m 
                                                               where m.dsc_mailing='".$nome_consultor."'";
							...
							$row = mysql_fetch_array($result);
                                                        ...
                                                        if(empty($mailing_pk)){
                                                            ...
                                                            $fields['dsc_mailing'] = $nome_consultor;
                                                            ...
                                                            $mailing_pk = mysql_insert_id();
                                                            
                                                            
                                                        }
                                                        
                                                        
							//VERIFICA SE O CONSULTOR TEM OU N�O PERMISS�O DE ALTERAR O MAILING								
								...
								if(!empty($_REQUEST['CodLead'])){		
									...
									if(!empty($mailing_pk)){
										...
										mysql_free_result($rss);
                                                                    ...
                                                                    if(permissao('mailing_nome_consultor', 'ic')){
                                                                        $sql ="";
                                                                        $sql.="SELECT m.pk, m.dsc_mailing
                                                                                   FROM mailing m
                                                                                        left join leads l on l.mailing_pk = m.pk
				...
				<tr>
					<td>&nbsp;<label for="codgerenteconta">Supervisor:</label></td>
					<td>
						<?
				...
				<tr>
					<td>&nbsp;<label for="codatendente">Consultor:</label></td>
					<td>
						<?
				...
				<tr>
					<td>&nbsp;<label for="codstatusclassificacaolead">Classifica��o:</label></td>
...
<tr>
	...
	?>
		...
		&nbsp;<input type="Radio" name="iluminado" value="2" <?=$checked2;?>>&nbsp;N�o Iluminado
	</td>
</tr>
<?}?>
				<tr>
               ...
               <tr>
					<td>&nbsp;<label for="ativacao">Data de ativa��o:</label></td>
					<td>
               ...
               <tr>
                   ...
                   <td>
                       <?
                        // $sql = "";
                         //$sql.="SELECT nco.pk classificacao_vivo_pk, nco.dsc_classificacao
                        //            FROM n_classificacao_operadora nco
                        //           WHERE nco.operadora_pk = 6";
                         
                        // combo($sql, "classificacao_vivo_pk", @$_REQUEST['classificacao_vivo_pk'], " ", "");
                       ?>
                   </td>
               </tr>
				<tr>
...
<tr>
		...
		<?if ($acao == ""){?>
			<tr>
				<td colspan="2">
					Informa��es Adicionais da Prospec��o
				</td>
			</tr>
			<tr>
				<td colspan="2">
...
<table class="form1" width="480"  border="0" cellpadding="0" cellspacing="0">
	<tbody>
		...
		<tr>
			<td>&nbsp;<label for="codfuncaocontato">Fun��o:</label></td>
			<td>
		...
		<tr>
			...
			<td>
				...
				<a href="#" onclick="consulta_np_telefone_contato()" title="Consultar bloqueio N�o Perturbe - Procon/SP"><img border="0" src="../../images/interrogacao.png"></a>
				<br>
		...
		<tr>
			...
			<td>
				...
				<a href="#" onclick="consulta_np_celular_contato()" title="Consultar bloqueio N�o Perturbe - Procon/SP"><img border="0" src="../../images/interrogacao.png"></a>
				<br>
