...
if(!empty($_REQUEST['coddocumento'])){
	$sql = "";
	$sql.="update documentos_usuario set ic_ativo = 0 where pk = ".$_REQUEST['coddocumento'];
	sql_query($sql);
...
<table id="tbl" name="tbl" border=0 cellpadding="1" cellspacing="1" width="100%">
	...
	$sql.="   from documentos_usuario d ";
	$sql.="		   inner join usuariosinternos ui on d.codusuariointerno_cadastro = ui.codusuariointerno ";
	$sql.=" where d.codusuariointerno = $codusuariointerno and ic_ativo = 1 ";
