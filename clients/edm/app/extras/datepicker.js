...
DatePicker.prototype.create = function ( doc ) {
	...
	// footer
	this._footer = doc.createElement( "div" );
	...
	this._footer.appendChild( footerTable );
	...
	this._todayButton.appendChild( doc.createTextNode( "Hoje" ) );
	...
	this._noneButton.appendChild( doc.createTextNode( "Nenhum" ) );
	...
	if ( !this._showNone && !this._showToday )
	...
	var dp = this;
	// buttons
	Event.observe(this._previousMonth, 'click', function(e) {
		this._dontChangeNone = true;
		this.goToPreviousMonth();
		this._dontChangeNone = false;
		Event.stop(e);
	}.bindAsEventListener(dp));
	Event.observe(this._nextMonth, 'click', function(e) {
		this._dontChangeNone = true;
		this.goToNextMonth();
		this._dontChangeNone = false;
		Event.stop(e);
	}.bindAsEventListener(dp));
	Event.observe(this._todayButton, 'click', function(e) {
		this.goToToday();
		Event.stop(e);
	}.bindAsEventListener(dp));
	Event.observe(this._noneButton, 'click', function(e) {
		this.setDate( null );
		return false;
		Event.stop(e);
	}.bindAsEventListener(dp));
	Event.observe(this._el, 'selectstart', function(e) {
		Event.stop(e);
	}.bindAsEventListener(dp));

	Event.observe(this._table, 'click', function(e) {
		// find td
		var el = Event.element(e)
		el = Event.findElement(e, 'td')
	...
	// show popup
	Event.observe(this._topLabel, 'click', function(e) {
		this._showLabelPopup();
		Event.stop(e);
	}.bindAsEventListener(dp));

	Event.observe(this._el, 'keydown', function(e) {
		var kc = e.keyCode != null ? e.keyCode : e.charCode;

		if ( kc < 37 || kc > 40 ) return true;

		var d = new Date( this._selectedDate ).valueOf();
		if ( kc == 37 ) // left
			d -= 24 * 60 * 60 * 1000;
		else if ( kc == 39 ) // right
			d += 24 * 60 * 60 * 1000;
		...
		this.setDate( new Date( d ), false );
		Event.stop(e);
	}.bindAsEventListener(dp));

	// ie6 extension
	Event.observe(this._el, 'mousewheel', function(e) {
...
DatePicker.prototype.setDate = function ( oDate, callEvent ) {

	this._hideLabelPopup();

	if ( callEvent == null || typeof callEvent == 'undefined' ) {
		callEvent = true;
	}
	...
	if ( oDate == null ) {
		if ( !this._none ) {
			...
			if ( typeof this.onchange == "function" && callEvent )
				this.onchange();
		}
		return;
	}
	...
	for ( i = 1; i < 32; i++ ) {
		...
		if ( tmpDate.getMonth() == this._selectedDate.getMonth() ) {
			...
			cells[currentWeek][colIndex].text =
				this._matrix[currentWeek][colIndex] = i;
...
DatePicker.prototype._setShowFooter = function ( ) {
	bShowFooter = this._showToday && this._showNone;

	if ( this._footer != null )
		this._footer.style.display = bShowFooter ? "" : "none";
	this._showFooter = bShowFooter;
}
