...
function paginainicial(){
    window.location = "transconsultor.php"
...
function exibirAjuda(){
	strMensagem ="";
	strMensagem+="Objetivo: Transferência de leads entre Consultores\n"
	strMensagem+="Abrangência: Supervisores de Comerciais, Gerentes e Administradores\n"
	strMensagem+="Funcionalidade: Transferir leads de Consultores que sairam da empresa, transferir leads para reciclar a carteira.\n"
	strMensagem+="Parâmetros:\n"
	strMensagem+=" - Consultores: Quando selecionado o operador, o sistema tranferirá TODOS os leads do determinado status selecionado (target, O%, etc...) deste operador para outro;\n"
	strMensagem+=" - Consultores: Quando selecionada a opção 'nenhum', o sistema transferirá APENAS os leads sem operador designado (carteira nova);\n"
...
<table width="100%" align="center"  height="5"  class="topo"  cellpadding="0" cellspacing="0">
	<tr>
		 <td  class="titulo">
			Tranferir Leads Consultor <?if(!empty($passo))print "Passo".$passo;?>
		</td>
	</tr>
</table>
								...
								<td>
									<table class="modulo1">
											...
											<td>
												<?	    $sql ="";
                                                        $sql.="select u.CodUsuarioInterno, u.Nome, u.Desativado 
                                                               from usuariosinternos u
                                                                 LEFT JOIN leads l ON u.CodUsuarioInterno = l.codgerenteconta
															     left join polo p on u.cod_polo = p.cod_polo                                                        
                                                               where gerentecontas = 1 
                                                               and l.codgerenteconta IS NOT NULL";
                                                        ...
                                                        $sql.=" group by u.codusuariointerno, u.nome, u.desativado
                                                                order by u.desativado, u.nome";
                                                ...
                                                //combo::consultor_equipe1($_SESSION['codusuario']);?>
                                                	<select id="codgerenteconta" name="codgerenteconta">                                                    
			                                        <option value="0">Nenhum</option>
			                                        <?
				                                        ...
				                                        while($row = mysql_fetch_array($result)){
                                                            ...
                                                            if($row["Desativado"] == 1)
                                                               ...
                                                               echo "<option value='".$row["CodUsuarioInterno"]."' style='color:#990000'>".$row["Nome"]."</option>";
			                                                else
			                                                    echo "<option value='".$row["CodUsuarioInterno"]."' style='color:#009900'>".$row["Nome"]."</option>";
			                                            }   
			                                            mysql_free_result($result);
			                                        ?>
			                                    </select>
										...
										<tr>
											...
											<td>
												<input type="Text" name="bairro" size="63" maxlength="63">
 											</td>
										</tr>                                        	
										<tr>
											<td>									
									...
									<table class="modulo1">
										<tr>
											...
											<td>
												<?			
                                                        $sql2 ="";
                                                        $sql2.="select u.CodUsuarioInterno, u.Nome, u.Desativado from usuariosinternos u where gerentecontas = 1 ";
                                                        if(!permissao('visualizar_todos_consultores', 'cs'))
                                                        $sql2.="   and u.codusuariointerno in (".equipes::getCodUsuariosEquipe($_SESSION['codusuario']).") ";
                                                        
                                                        if($_SESSION['cod_polo'] > 0){
                                                        $sql2.="  and u.cod_polo = ".$_SESSION['cod_polo']." ";
                                                        }
                                                        
                                                        $sql2.=" order by u.desativado, u.nome ";
															...
															$tipos[1]['style'] = 'color:#990000';
															$tipos['max'] = 2;										
													combo_tipos( $sql2 , "codconsultorpara" , $tipos , "" , " " , "" ) ;
												?>
											</td>
										</tr>
									</table>			
...
if($passo==4){	
	...
	$dt_ult_ocorrencia = $_REQUEST['dt_ult_ocorrencia'];

	$total_transferido = "";
	$selecao = $_REQUEST['selecao'];

	$codconsultor = $_REQUEST['codconsultorpara'];
	$codlead = $_REQUEST['codlead'];
	//DADOS USUARIO

	$fields['codgerenteconta'] = $codconsultor ;
	$fields1['CodUsuarioInterno'] = $codconsultor;
	$fields2['AgendadoPara'] = $codconsultor;
	$fields2['CodUsuarioInterno'] = $codconsultor;
    
	$sql = "Select 
				ui.nome
				,ui.cod_polo
			from usuariosinternos ui 
			where codusuariointerno=".$_REQUEST['codconsultorpara'];

  	$rs = sql_query($sql);
    $row = mysql_fetch_array($rs);
    $Para = $row['nome'];
	if(!empty($row['cod_polo'])){
		$Consultor_polo['cod_polo'] = $row['cod_polo'];
	}else{
		$Consultor_polo['cod_polo'] = "0";
	}
...
//if($selecao==2){
	...
	for($i = 0; $i<count($codlead);$i++){	
		...
		$sql = "Select
				ui.nome 
			from leads l
			inner join usuariosinternos ui on l.codgerenteconta = ui.codusuariointerno			
		...
		//$sql = sqlupdate('leads', $Consultor_polo, ' CodLead = ' . mysqlnull($codlead[$i]).' and cod_polo=0');
		//sql_query($sql);	
        
        //ALTERA CONSULTOR DA VISITA
        $sql = "Select
                    a.codagendalead
                from agendaslead a
                where a.codlead=".$codlead[$i];
        $sql .= " and a.Termino is null";
              
        $result = sql_query($sql);        
        if($ag = mysql_fetch_array($result)){
               $sql = sqlupdate('agendagerenteconta', $fields, ' codagendalead = ' . mysqlnull($ag['codagendalead']));	
               sql_query($sql);
        }    	

		//VERIFICA AS OCORRENCIAS DO QUE SAO DO CONSULTOR
		$sql = "Select   
		...
		while($rss1 = mysql_fetch_array($result3)){					
			...
			if(!empty($rss1['dt_retorno'])){
				if(empty($rss1['dt_retorno_fechamento'])){
					$fielduser['agendadopara']=$codconsultor;
		...
		//INCLUI A OCORRENCIA
		if(!empty($De['nome'])){
			$descricaoreagendamento = "Transferência de Lead Consultor=".$De['nome']." para o Atendente=".$Para ;
		}else{
			$descricaoreagendamento = "Transferência de Lead para Consultor=".$Para ;
		}
	...
	foreach($status as $stat => $qtd){
		if($qtd > 0){
			...
			while($row = mysql_fetch_array($result1)){		
				...
				//UPDATE POLO LEAD PARA O DO ATENDENTE				
				//$sql = sqlupdate('leads', $Atendente_polo, ' CodLead = ' . mysqlnull($row['codlead']));	
                //sql_query($sql);
			
				//VERIFICA AS OCORRENCIAS DO QUE SAO DO GERENTE
				$sql = "Select   
						  oc.CodOcorrenciaLead
						  ,oc.dt_retorno
						  ,oc.dt_retorno_fechamento
				...
				while($rss1 = mysql_fetch_array($result3)){	
					...
					$sql = sqlupdate('ocorrenciaslead', $fields1, ' CodOcorrenciaLead = ' . mysqlnull($rss1['CodOcorrenciaLead']));
   					sql_query($sql);					
                        //ALTERA O USUARIO DO RETORNO EM ABERTO
						if(!empty($rss1['dt_retorno'])){
							if(empty($rss1['dt_retorno_fechamento'])){
								$fielduser['agendadopara']=$codconsultor;
								$sql = "update ocorrenciaslead set agendadopara=".$codconsultor." where CodOcorrenciaLead=".$rss1['CodOcorrenciaLead']." and agendadopara=".$rss1['codusuariointerno'];                                
								sql_query($sql);
							}
						}					
				}	
                
				//DESCRICAO OCORRENCIA TRANSFERENCIA
				if(!empty($De['nome'])){
					$descricaoreagendamento = "Transferência de Lead Consultor=".$De['nome']." para o Consultor=".$Para ;
				}else{
					$descricaoreagendamento = "Transferência de Lead para Consultor=".$Para ;
				}
                //INCLUI OCORRENCIA DE TRANSFERENCIA
