...
function limparModalReloginho(){
    $("#grid_consulta_folha_ponto").html("");
    $("#text_folha_finalizada").html("");
    $("#ponto_folha_pk").val("");
    $("#ic_status_ponto_folha_pk").val("");
    $("#agenda_consulta_folha_colaborador_pk").val("");
    $("#leads_consulta_folha_pk").val("");
    $("#colaborador_consulta_folha_pk").val("");
    $("#dt_ini_reloginho").val("");
    $("#dt_fim_reloginho").val("");
    $("#ds_turno").html("");
    $("#periodo_trabalho").html("");
    $("#text-colaborador").html("");
    $(".print_folha").removeClass("btn-outline-warning btn-danger").addClass("btn-warning").prop("disabled", false);
    $(".finalizar_folha").show().prop("disabled", false);
    $("#consulta_folha_ponto .modal-body").scrollTop(0);
}

function aplicarPeriodoPadraoReloginho(){
    var hoje = new Date();
    var periodoReloginho = calcularPeriodoReloginho(hoje);
    var dtInicioReloginho = formatDate(periodoReloginho.inicio);
    var dtFimReloginho = formatDate(periodoReloginho.fim);

    $("#dt_ini_reloginho").datepicker("setDate", periodoReloginho.inicio).val(dtInicioReloginho);
    $("#dt_fim_reloginho").datepicker("setDate", periodoReloginho.fim).val(dtFimReloginho);
}
...
function validarPeriodoUmMes(dtIni, dtFim) {
    var periodo = calcularPeriodoPermitidoReloginho(dtFim);

    return dtIni.getFullYear() == periodo.inicio.getFullYear()
        && dtIni.getMonth() == periodo.inicio.getMonth()
        && dtIni.getDate() == periodo.inicio.getDate()
        && dtFim.getFullYear() == periodo.fim.getFullYear()
        && dtFim.getMonth() == periodo.fim.getMonth()
        && dtFim.getDate() == periodo.fim.getDate();
}

function mensagemPeriodoPermitidoReloginho(dtFim) {
    var periodo = calcularPeriodoPermitidoReloginho(dtFim);
    return "O período permitido para este mês é de " + formatDate(periodo.inicio) + " até " + formatDate(periodo.fim) + ".";
}

function validarPeriodoReloginho() {
    if ($('#dt_ini_reloginho').val() == "" || $('#dt_fim_reloginho').val() == "") {
        sweetMensagem('warning','Por favor preencha o período!');
        return false;
    }
    ...
    if (!validarPeriodoUmMes(dtIni, dtFim)) {
        sweetMensagem('warning', mensagemPeriodoPermitidoReloginho(dtFim));
        return false;
    }
...
function estiloSituacaoReloginho(situacao, possuiApontamento) {
    if ($.trim(situacao).toLowerCase() == "falta") {
        return "text-align: center;background-color:#f8c8dc";
    }

    if (possuiApontamento) {
        return "text-align: center;background-color:#ADD8E6";
    }

    return "text-align: center";
}

function fcCarregarGridPontoFolhaProcessar(){
    $("#grid_consulta_folha_ponto").html("");
    $("#grid_consulta_folha_ponto").append("");
        ...
        if(arrCarregar.data.length>0){
            ...
            for(i=0;i<arrCarregar.data.length;i++){
                ...
                if(arrCarregar.data[i]['pontos_dia'][0]['ic_apontamento']!=0){
                    ...
                    html += "    </th>";
                    html+='<th  style="' + estiloSituacaoReloginho(arrCarregar.data[i]['pontos_dia'][0]['situacao'], true) + '">';
                    ...
                    html += '     <a class="reloginho-action-icon reloginho-action-delete" onclick="fcExcluirApontamento(' + arrCarregar.data[i]['pontos_dia'][0]['apontamento_pk'] + ')" title="Excluir apontamento"><i class="bi bi-trash3-fill"></i></a>';
                    html += '     <a class="reloginho-action-icon reloginho-action-history" onclick="abrirModalHistorico(' + i + ')" title="Histórico de ponto"><i class="bi bi-clock-history"></i></a>';
                    html += '     <a class="reloginho-action-icon reloginho-action-image" onclick="fcPontoDiario(' + i + ')" title="Ver imagem do ponto"><i class="bi bi-image"></i></a>';
                    html += '</th>';
                    ...
                    html += "<input disabled type='type' id='hr_faltantes" + i + "' size='3' value='" + arrCarregar.data[i]['pontos_dia'][0]['hr_faltante'] + "' onkeypress='mascara(this,horamask)' >";
                    html += "    </th>";
                    html+='<th  style="' + estiloSituacaoReloginho(arrCarregar.data[i]['pontos_dia'][0]['situacao'], false) + '">';
                    html += arrCarregar.data[i]['pontos_dia'][0]['situacao'] ;
                    html+='</th>';
                    html+='<th  style="  text-align: center">';
                    html+='<select class="form-control form-control-sm" id="tipo_ponto_pk'+i+'" onchange="habilitarCampos('+i+',this.value)">';
                    ...
                    html+='</th>';
                    html+='<th  style="  text-align: center">';
                    html += '     <a class="reloginho-action-icon reloginho-action-save" onclick="fcSalvarApontamentoReloginho('+i+')" title="Salvar apontamento"><i class="fa fa-save"></i></a>';
                    html += '     <a class="reloginho-action-icon reloginho-action-history" onclick="abrirModalHistorico(' + i + ')" title="Histórico de ponto"><i class="bi bi-clock-history"></i></a>';
                    html += '     <a class="reloginho-action-icon reloginho-action-image" onclick="fcPontoDiario(' + i + ')" title="Ver imagem do ponto"><i class="bi bi-image"></i></a>';
                    html+='</th>';
    ...
    if($("#ic_status_ponto_folha_pk").val()==1){
        ...
        $("#text_folha_finalizada").html('<div class="reloginho-finalizada-aviso">Esta folha já está finalizada e não pode ser alterada.</div>');
    }
    else{
        $(".finalizar_folha").show().prop("disabled", false);
...
function fcCarregarGridPontoFolha(){
    utilsJS.loading("Carregando acompanhamento de ponto...");

    setTimeout(function(){
        try {
            fcCarregarGridPontoFolhaProcessar();
        } finally {
            utilsJS.loaded();
        }
    }, 100);
...
function abrirModalHistorico(i){
    ...
    if ($.fn.DataTable.isDataTable("#tblListarPontoDia")) {
...
function listarHistorico(i){
    ...
    if(i>=0){
        ...
        $("#tblListarPontoDia").off("error.dt").on("error.dt", function () {
            return false;
        });
    ...
    if (arrCarregarP.status == true){
        if(arrCarregarP.data!=null){
            if(arrCarregarP.data.length > 0){
                strRetorno+="<table class='reloginho-modal-table table table-striped table-no-bordered display nowrap' style='width:100%'>";
...
function calcularPeriodoReloginho(dataBase) {
    return calcularPeriodoPermitidoReloginho(dataBase);
}

function calcularPeriodoPermitidoReloginho(dataBase) {
    var inicioRegraNova = new Date(2026, 4, 1);
    var mesVigente = new Date(dataBase.getFullYear(), dataBase.getMonth(), 1);
    var diasMesVigente = new Date(dataBase.getFullYear(), dataBase.getMonth() + 1, 0).getDate();

    if (mesVigente < inicioRegraNova) {
        return {
            inicio: new Date(dataBase.getFullYear(), dataBase.getMonth(), 1),
            fim: new Date(dataBase.getFullYear(), dataBase.getMonth(), diasMesVigente)
        };
    }

    var dtInicio = new Date(dataBase.getFullYear(), dataBase.getMonth() - 1, 21);
    var dtFim = new Date(dtInicio.getFullYear(), dtInicio.getMonth(), dtInicio.getDate());

    dtFim.setDate(dtFim.getDate() + diasMesVigente - 1);
...
$(document).ready(function () {
    ...
    var dtFimReloginho = formatDate(periodoReloginho.fim);
     ...
     $('#dt_ini_reloginho').datepicker({
        ...
        todayHighlight: true,
        todayBtn: "linked"
    }).datepicker('setDate', periodoReloginho.inicio).val(dtInicioReloginho);
    ...
    $('#dt_fim_reloginho').datepicker({
        ...
        todayHighlight: true,
        todayBtn: "linked"
    }).datepicker('setDate', periodoReloginho.fim).val(dtFimReloginho);
