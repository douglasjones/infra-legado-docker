function fcSalvar() {
    try{
        ...
        var v_n_qtde_dias_semana = fcValorEscalaServico();

        if (arrCarregar0 && arrCarregar0.status == true && arrCarregar0.data.length > 0) {
            if (arrCarregar0.data[0]['contratos_itens_pk']) {
                v_contratos_itens_pk = arrCarregar0.data[0]['contratos_itens_pk'];
            }
            if (arrCarregar0.data[0]['n_qtde_dias_semana']) {
        ...
        var objParametros = {
            ...
            "dias_escala_servico": v_n_qtde_dias_semana,
        ...
        if (arrEnviar && arrEnviar.status == true) {
            ...
            sweetMensagem('warning', arrEnviar.message || arrEnviar.result || 'Falha ao salvar escala.');
        }
    }catch(e){
...
function fcCadastrarEscala() {
    ...
    if (arrEnviar && arrEnviar.status == true) {
        ...
        sweetMensagem('warning',arrEnviar.result);
    }
}

function fcValorEscalaServico() {
    var valor = $("#n_qtde_dias_semana").val();

    if (!valor) {
        valor = $("#dias_escala_servico").val();
    }
...
function fcLimparAlertaEscalaColaborador() {
    $("#print_escala_colaborador").html('');
    $("#cmdSalvar, #cmdEnviarAgenda").show();
}

function fcVerificaOutraEscalaColaborador() {
    var vhtml = "";
    var colaborador_pk = $("#agenda_colaboradores_pk").val();

    fcLimparAlertaEscalaColaborador();
    ...
    if (arrCarregar && arrCarregar.status == true) {
        ...
        if (arrCarregar.data && arrCarregar.data.length > 0 && arrCarregar.data[0]['pk'] != 0) {

            $("#cmdSalvar, #cmdEnviarAgenda").hide();
