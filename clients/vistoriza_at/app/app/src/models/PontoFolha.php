...
class PontoFolha {
    ...
    public function listarGrid($empresas_pk, $leads_pk){
        ...
        if(count($query) > 0){
            for($i=0; $i<count($query);$i++){
                $sql ="";
                $sql.=" SELECT CASE";
                $sql.="            WHEN DAY(dt_periodo_ini) = 21 AND dt_periodo_fim >= '2026-05-01' THEN YEAR(dt_periodo_fim)";
                $sql.="            ELSE YEAR(dt_periodo_ini)";
                $sql.="        END ano_folha";
                $sql.="   FROM ponto_folha pf";
                $sql.="  WHERE pf.leads_pk =". $query[$i]['lead_pk'];
                $sql.="  GROUP BY ano_folha";
                ...
                for($l=0; $l<count($ano);$l++){
                    $sql ="";
                    $sql.=" SELECT CASE";
                    $sql.="            WHEN DAY(dt_periodo_ini) = 21 AND dt_periodo_fim >= '2026-05-01' THEN MONTH(dt_periodo_fim)";
                    $sql.="            ELSE MONTH(dt_periodo_ini)";
                    $sql.="        END mes_folha";
                    $sql.="  ,CASE";
                    $sql.="      WHEN DAY(dt_periodo_ini) = 21 AND dt_periodo_fim >= '2026-05-01' THEN";
                    $sql.="        CASE MONTHNAME(pf.dt_periodo_fim)";
                    ...
                    $sql.="  WHERE pf.leads_pk =". $query[$i]['lead_pk'];
                    $sql.="    AND CASE";
                    $sql.="            WHEN DAY(pf.dt_periodo_ini) = 21 AND pf.dt_periodo_fim >= '2026-05-01' THEN YEAR(pf.dt_periodo_fim)";
                    $sql.="            ELSE YEAR(pf.dt_periodo_ini)";
                    $sql.="        END = ".$ano[$l]['ano_folha'];
                    $sql.="  GROUP BY mes_folha";
                    $sql.="  ORDER BY mes_folha";
                    $stmt = $this->pdo->prepare( $sql );
                    $stmt->execute();
                    ...
                    for($a=0;$a<count($mes);$a++){
                        ...
                        $sql.="        END = ".$ano[$l]['ano_folha'];
                        $sql.="    AND CASE";
                        $sql.="            WHEN DAY(pf.dt_periodo_ini) = 21 AND pf.dt_periodo_fim >= '2026-05-01' THEN MONTH(pf.dt_periodo_fim)";
                        $sql.="            ELSE MONTH(pf.dt_periodo_ini)";
                        $sql.="        END = ".$mes[$a]['mes_folha'];
                        $stmt = $this->pdo->prepare( $sql );
                        $stmt->execute();
    ...
    public function desabilitarApontamentosAtivosDia($colaborador_pk, $leads_pk, $dt_apontamento){
        $dt_apontamento = Util::DataYMD($dt_apontamento);
        ...
        $where.= " AND leads_pk = ".$leads_pk;
        $where.= " AND DATE(dt_apontamento) = '".$dt_apontamento."'";
        $where.= " AND ic_status = 1";

        Util::execUpdate("agenda_colaborador_apontamento", $fields, $where, $this->pdo);
    }

    public function salvarApontamentoFolha($dadosRegistro){
        if(!isset($dadosRegistro['apontamento_alterado']) || $dadosRegistro['apontamento_alterado']!=1){
            return;
        }
        if(!isset($dadosRegistro['tipo_apontamento_pk_novo']) || $dadosRegistro['tipo_apontamento_pk_novo']==""){
            return;
        }

        $tipo_apontamento_pk = $dadosRegistro['tipo_apontamento_pk_novo'];
        $agenda_colaborador_pk = isset($dadosRegistro['agenda_colaborador_pk']) ? $dadosRegistro['agenda_colaborador_pk'] : "";
        if($agenda_colaborador_pk==""){
            return;
        }

        $this->desabilitarApontamentosAtivosDia(
            $dadosRegistro['colaborador_pk'],
            $dadosRegistro['leads_pk'],
            $dadosRegistro['dt_hora_ponto']
        );

        $entity = new AgendaColaboradorApontamento($this->pdo);
        $apontamento = [
            "pk"=>"",
            "leads_pk"=>$dadosRegistro['leads_pk'],
            "tipo_apontamento_pk"=>$tipo_apontamento_pk,
            "colaborador_pk"=>$dadosRegistro['colaborador_pk'],
            "agenda_colaborador_padrao_pk"=>$agenda_colaborador_pk,
            "dt_apontamento"=>Util::DataYMD($dadosRegistro['dt_hora_ponto']),
        ];

        $retorno = $entity->salvar($apontamento);
        if(!isset($retorno->status) || $retorno->status!=true){
            return;
        }
        ...
        $apontamentoDetalhe = [
            ...
            "dt_apontamento"=>$dadosRegistro['dt_hora_ponto'],
            "hr_ini_expediente"=>isset($dadosRegistro['hr_ini_expediente']) ? $dadosRegistro['hr_ini_expediente'] : "",
            "hr_ini_intervalo"=>isset($dadosRegistro['hr_ini_intervalo']) ? $dadosRegistro['hr_ini_intervalo'] : "",
            "hr_fim_intervalo"=>isset($dadosRegistro['hr_fim_intervalo']) ? $dadosRegistro['hr_fim_intervalo'] : "",
            "hr_fim_expediente"=>isset($dadosRegistro['hr_fim_expediente']) ? $dadosRegistro['hr_fim_expediente'] : "",
            "hr_trabalhadas"=>isset($dadosRegistro['hr_trabalhadas']) ? $dadosRegistro['hr_trabalhadas'] : "",
            "hr_excedentes"=>isset($dadosRegistro['hr_excedentes']) ? $dadosRegistro['hr_excedentes'] : "",
            "hr_faltantes"=>isset($dadosRegistro['hr_faltantes']) ? $dadosRegistro['hr_faltantes'] : "",
            "motivo_falta_pk"=>3,
            "motivo_afastamento_pk"=>1,
            "feriado_pk"=>"",
        ];

        $entity->salvarApontamentoReloginho($apontamentoDetalhe);
    }
    ...
    public function alterarRegistrosFolhaPonto($jsonDadosRegistros){
        ...
        try{
            ...
            Util::execDelete('ponto_folha_registros', $whereRegistros, $this->pdo);
            for($i=0;$i<count($arrDadosRegistros);$i++){
                $this->salvarApontamentoFolha($arrDadosRegistros[$i]);
    ...
    public function listarDiasEscalaPorColaborador($colaborador_pk,$dt_periodo_ini,$dt_periodo_fim){
    ...
    public function completarDiasEscalaPeriodo($diasEscala, $colaborador_pk, $dt_periodo_ini, $dt_periodo_fim){
        $diasPorData = [];

        for($i = 0; $i < count($diasEscala); $i++){
            $diasPorData[$diasEscala[$i]['dt_escala']] = $diasEscala[$i];
        }

        $periodoInicio = new \DateTime($dt_periodo_ini);
        $periodoFim = new \DateTime($dt_periodo_fim);
        $periodoFim->modify('+1 day');
        $periodo = new \DatePeriod($periodoInicio, new \DateInterval('P1D'), $periodoFim);
        $diasSemana = [
            1 => 'Seg',
            2 => 'Ter',
            3 => 'Qua',
        ...
        foreach($periodo as $dia){
            $dtEscala = $dia->format('Y-m-d');

            if(isset($diasPorData[$dtEscala])){
                $diasCompletos[] = $diasPorData[$dtEscala];
                continue;
            }

            $escala = $this->pegarHorarioDeEntradaPorDataDiaSemana($colaborador_pk, $dtEscala);
            $icDiaAtivo = isset($escala['dados']['ic_dia_ativo']) ? (int)$escala['dados']['ic_dia_ativo'] : 2;

            $diasCompletos[] = [
                'ic_escala' => $icDiaAtivo == 1 ? 1 : 2,
                'dt_escala' => $dtEscala,
                'tipo_escala_pk' => '',
                'dia_da_semana' => $diasSemana[(int)$dia->format('N')],
                'dt_format' => $dia->format('d/m/Y')
            ];
        }
    ...
    public function listarFolhaRegistrosAgrupadoData($ponto_folha_pk,$colaborador_pk){
        ...
        $sql.=" ORDER BY CASE WHEN DAY(pf.dt_periodo_ini) = 21 AND pf.dt_periodo_fim >= '2026-05-01' THEN pfr.dt_hora_ponto ELSE pfr.pk END";
  ...
  public function pegarHorarioDeEntradaPorDataDiaSemana($colaboradores_pk, $dt_escala)
    {
        try{
    ...
    public function garantirFolhaReloginho($leads_pk, $colaboradores_pk, $dt_periodo_ini, $dt_periodo_fim, $agenda_colaborador_padrao_pk, $arrDias){
        $ponto_folha = $this->getPontoFolhaByPeriodoLead($leads_pk, $dt_periodo_ini, $dt_periodo_fim);
        $ponto_folha_pk = isset($ponto_folha['pk']) ? $ponto_folha['pk'] : "";

        if($ponto_folha_pk==""){
            $dados = (new Colaborador($this->pdo))->listarPk($colaboradores_pk);

            $fields = array();
            $fields['empresas_pk'] = isset($dados->data[0]['empresas_pk']) ? $dados->data[0]['empresas_pk'] : "";
            $fields['dt_periodo_ini'] = $dt_periodo_ini;
            $fields['dt_periodo_fim'] = $dt_periodo_fim;
            ...
            $ponto_folha_pk = Util::execInsert("ponto_folha", $fields, $this->pdo);
        }

        $ponto_folha_colaborador = $this->getPontoFolhaColaborador($ponto_folha_pk, $colaboradores_pk);
        if(count($ponto_folha_colaborador)==0){
            $fields = array();
            $fields['ponto_folha_pk'] = $ponto_folha_pk;
            $fields['colaborador_pk'] = $colaboradores_pk;
            $fields["dt_ult_atualizacao"] = "sysdate()";
        ...
        if(isset($ponto_folha_colaborador[0]['ic_status']) && $ponto_folha_colaborador[0]['ic_status']==1){
            return array(
                "pk" => $ponto_folha_pk,
                "ic_status" => $ponto_folha_colaborador[0]['ic_status']
            );
        }

        for($i=0;$i<count($arrDias);$i++){
            if(!isset($arrDias[$i]['pontos_dia'][0])){
                continue;
            }

            $registroExistente = $this->getRegistroFolhaPorData($ponto_folha_pk, $colaboradores_pk, $arrDias[$i]['dt_hora_ponto_usa']);
            if(isset($registroExistente['ic_status']) && $registroExistente['ic_status']==1){
                continue;
            }

            if(isset($registroExistente['pk']) && $registroExistente['pk']!=""){
                Util::execDelete('ponto_folha_registros', ' pk='.$registroExistente['pk'], $this->pdo);
            }

            $pontosDia = $arrDias[$i]['pontos_dia'][0];
            $fields = array();
            $fields['dt_hora_ponto'] = $arrDias[$i]['dt_hora_ponto_usa'];
            $fields['colaborador_pk'] = $colaboradores_pk;
            $fields['ponto_folha_pk'] = $ponto_folha_pk;
            $fields['hr_ini_expediente'] = $pontosDia['ponto_ini_expediente'];
            $fields['hr_ini_intervalo'] = $pontosDia['ponto_ini_intervalo'];
            $fields['hr_fim_intervalo'] = $pontosDia['ponto_term_intervalo'];
            $fields['hr_fim_expediente'] = $pontosDia['ponto_term_expediente'];
            $fields['hr_trabalhadas'] = $pontosDia['horas_trabalhadas'];
            $fields['hr_excedente'] = $pontosDia['hr_excedentes'];
            $fields['hr_faltantes'] = $pontosDia['hr_faltante'];
            $fields['tipo_ponto_pk'] = $pontosDia['tipo_ponto_pk'];
            $fields['obs'] = isset($pontosDia['obs']) ? $pontosDia['obs'] : "";
            $fields["dt_ult_atualizacao"] = "sysdate()";
            $fields["usuario_ult_atualizacao_pk"] = $_SESSION['session_user']['par1'];
            $fields["dt_cadastro"] = "sysdate()";
            $fields["usuario_cadastro_pk"] = $_SESSION['session_user']['par1'];
        ...
        return array(
            "pk" => $ponto_folha_pk,
            "ic_status" => isset($ponto_folha_colaborador[0]['ic_status']) ? $ponto_folha_colaborador[0]['ic_status'] : ""
        );
    }

    public function getPontoFolhaByPeriodoLead($leads_pk, $dt_periodo_ini, $dt_periodo_fim){
        $sql ="";
        $sql.=" SELECT pf.pk";
        $sql.=" FROM ponto_folha pf";
        $sql.=" WHERE pf.leads_pk = ".$leads_pk;
        $sql.="   AND pf.dt_periodo_ini = '".$dt_periodo_ini."'";
        $sql.="   AND pf.dt_periodo_fim = '".$dt_periodo_fim."'";
    ...
    public function getOuCriarPontoFolhaPkPeriodo($leads_pk, $colaborador_pk, $dt_periodo_ini, $dt_periodo_fim){
        $ponto_folha = $this->getPontoFolhaByPeriodoLead($leads_pk, $dt_periodo_ini, $dt_periodo_fim);
        if(isset($ponto_folha['pk']) && $ponto_folha['pk']!=""){
            return $ponto_folha['pk'];
        }
        ...
        return Util::execInsert("ponto_folha", $fields, $this->pdo);
    }

    public function getPontoFolhaColaborador($ponto_folha_pk, $colaboradores_pk){
        $sql ="";
        $sql.=" SELECT pk, ic_status";
        $sql.=" FROM ponto_folha_colaborador";
        $sql.=" WHERE ponto_folha_pk = ".$ponto_folha_pk;
    ...
    public function getRegistroFolhaPorData($ponto_folha_pk, $colaboradores_pk, $dt_hora_ponto){
        $sql ="";
        $sql.=" SELECT pk, ic_status";
        $sql.=" FROM ponto_folha_registros";
        $sql.=" WHERE ponto_folha_pk = ".$ponto_folha_pk;
    ...
    public function listarConsultaPontoColaborador($leads_pk, $colaboradores_pk, $dt_periodo_ini,$dt_periodo_fim,$agenda_colaborador_padrao_pk){              
        ...
        $diasEscala = $this->listarDiasEscalaPorColaborador($colaboradores_pk,$dt_periodo_ini, $dt_periodo_fim);
        $diasEscala = $this->completarDiasEscalaPeriodo($diasEscala, $colaboradores_pk, $dt_periodo_ini, $dt_periodo_fim);
        $turnos_pk = $this->listarTurnosPk($agenda_colaborador_padrao_pk);
        ...
        for($i=0; $i<count($diasEscala); $i++){  
            ...
            $escala = $this->pegarHorarioDeEntradaPorDataDiaSemana($colaboradores_pk, $dt_escala);
        ...
        $arrPontoFolha = $this->garantirFolhaReloginho($leads_pk, $colaboradores_pk, $dt_periodo_ini, $dt_periodo_fim, $agenda_colaborador_padrao_pk, $arrDias);
        for($i=0;$i<count($arrDias);$i++){
            $arrDias[$i]['ponto_folha_pk'] = isset($arrPontoFolha['pk']) ? $arrPontoFolha['pk'] : "";
            $arrDias[$i]['ic_status_ponto_folha_pk'] = isset($arrPontoFolha['ic_status']) ? $arrPontoFolha['ic_status'] : "";
        }
    ...
    public function gerarFolhaPontoByRelogio($arrDados){
        ...
        try{
            ...
            $dtPeriodoIniReloginho = Util::DataYMD($arrDados[0]['dt_inicio']);
            $dtPeriodoFimReloginho = Util::DataYMD($arrDados[0]['dt_fim']);
            ...
            $primeiroDiaMesAtual = $dtPeriodoIniReloginho;
            $ultimoDiaMesAtual = $dtPeriodoFimReloginho;

          
            //if($arrDados[0]['ic_status_ponto_folha_pk']!=1){
                //Verificar se existe a folha 
                $ponto_folha_pk = $this->getOuCriarPontoFolhaPkPeriodo($arrDados[0]['leads_pk'],$arrDados[0]['colaborador_pk'],$primeiroDiaMesAtual,$ultimoDiaMesAtual);
