...
class Documento {
    ...
    public function pegarDocumentoBd($pk_doc_bd){
        ...
        $retorno->data = []; //Retorno data setado como vazio
        $sql ="";
        $sql.="select td.docsId, td.docsType, td.docsData, d.ds_nome_original, d.ds_documento ";
        $sql.="from tbl_docs td ";
        $sql.="left join documentos d on d.pk_doc_bd = td.docsId ";
        $sql.="where td.docsId = :pk_doc_bd ";
        $sql.="order by d.pk desc ";
        $sql.="limit 1 ";

        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':pk_doc_bd', $pk_doc_bd, \PDO::PARAM_INT);
        $stmt->execute();
        $rows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
