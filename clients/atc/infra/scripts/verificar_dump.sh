...
assert_usuarios_definition() {
  ...
  if [[ "${dump_path}" == *.gz ]]; then
    if gzip -cd "${dump_path}" | awk '/CREATE TABLE .*usuarios/{found=1; exit} END{exit !found}'; then
    ...
    if gzip -cd "${dump_path}" | awk '/INSERT INTO .*usuarios/{found=1; exit} END{exit !found}'; then
