...
class Lead {
    ...
    public function buscarPorDescricao($ds_lead){
        $sql = "";
        $sql .= " select pk, ds_lead ";
        $sql .= " from leads ";
        $sql .= " where ds_lead = :ds_lead ";
        $sql .= " order by pk asc ";
        $sql .= " limit 1 ";

        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':ds_lead', $ds_lead);
        $stmt->execute();

        return $stmt->fetch(\PDO::FETCH_ASSOC);
    }

    private function importarQrCodeLegado($leads_pk){
        if($leads_pk == ""){
            return;
        }
        ...
        $sql .= " select ds_lead ";
        $sql .= " from leads ";
        $sql .= " where pk = :leads_pk ";
        $sql .= " limit 1 ";

        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':leads_pk', $leads_pk);
        $stmt->execute();
        ...
        $sql = "";
        $sql .= " select distinct trim(local_ronda_pk) ds_ponto ";
        $sql .= " from ronda ";
        $sql .= " where leads_pk = :ds_lead ";
        $sql .= "   and local_ronda_pk is not null ";
        $sql .= "   and trim(local_ronda_pk) <> '' ";
        $sql .= " order by trim(local_ronda_pk) asc ";

        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':ds_lead', $lead['ds_lead']);
        $stmt->execute();
        ...
        foreach($rows as $row){
            ...
            $fields['ds_ponto'] = $row['ds_ponto'];
            $fields["dt_ult_atualizacao"] = "sysdate()";
            $fields["usuario_ult_atualizacao_pk"] = isset($_SESSION['session_user']['par1']) ? $_SESSION['session_user']['par1'] : 1;
            $fields["dt_cadastro"] = "sysdate()";
            $fields["usuario_cadastro_pk"] = isset($_SESSION['session_user']['par1']) ? $_SESSION['session_user']['par1'] : 1;

            Util::execInsert("lead_ronda_qrcode", $fields,$this->pdo);
        }
    }
    ...
    public function listarQRCode($leads_pk){
        ...
        if(count($rows) == 0 && $leads_pk != ""){
            $this->importarQrCodeLegado($leads_pk);
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute();
            $rows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
        }
