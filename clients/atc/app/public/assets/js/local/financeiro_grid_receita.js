...
function fcPeriodoReceita(campo) {
    let nome = 'date_range_filter_' + campo + '_receita';
    let valorCampo = $.trim($('input[name="' + nome + '"]').val());
    if (valorCampo !== '') {
        localStorage.setItem(nome, valorCampo);
        return valorCampo;
    }
    return $.trim(localStorage.getItem(nome) || '');
}
...
function fcCarregarGridReceita(){
    ...
    let objParametros = {
        ...
        'dt_cadastro': fcPeriodoReceita('cadastro'),
        'dt_faturamento': fcPeriodoReceita('faturamento'),
        'dt_vencimento': fcPeriodoReceita('vencimento'),
        'dt_pagamento': fcPeriodoReceita('pagamento'),
        'empresas_pk': $('#empresa_receita_pk').val(),
    ...
    if(arrCarregarReceita.data.length > 0){
        ...
        $('#tblReceita').append('<tbody><tr><td colspan="16" align="center">Nenhum registro encontrado</td></tr></tbody>');
    }
...
function fcCarregarFuncoesReceita(){
    ...
    fcCarregarGridReceita();
    $(document).off('click', '#cmdPesqReceita').on('click', '#cmdPesqReceita', fcCarregarGridReceita);
