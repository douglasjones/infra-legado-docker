...
var envioRegistroPontoEmAndamento = false;

function iniciarEnvioRegistroPonto() {
    envioRegistroPontoEmAndamento = true;
    utilsJS.loading('Registrando ponto, aguarde...');
}

function finalizarEnvioRegistroPonto() {
    envioRegistroPontoEmAndamento = false;
    utilsJS.loaded();
}

function retornarTelaInicialRegistroPonto() {
    sendPost("area_colaborador", "receptivo", {});
}

function fcSalvarPontoManual(tipo_ponto){
    if (envioRegistroPontoEmAndamento) {
        return false;
    }

   
    if(tipo_ponto==1){
        v_descricao = "Entrada Expediente";
    }
    else if(tipo_ponto==3){
        v_descricao = "Saida Intervalo";
    }
    else if(tipo_ponto==4){
        v_descricao = "Retorno Intervalo";
    }
    else if(tipo_ponto==2){
        v_descricao = "Saida Expediente";
    }
    utilsJS.sweetMensagemConfirm('Ponto', 'Deseja bater o ponto de '+v_descricao+' ?', function () {
        iniciarEnvioRegistroPonto();
        const photoDataUrl = webcamCanvas.toDataURL("image/png");
            ...
            $.ajax({
                ...
                complete: function (response) {
                    try {
                        var log = JSON.parse(response.responseText);
                        if(log.status==true) {
                            retornarTelaInicialRegistroPonto();
                        }
                        else{
                            finalizarEnvioRegistroPonto();
                            utilsJS.toastNotify(false,log.message);
                        }
...
function fcSalvarPonto(tipo_ponto){
    ...
    if($("#leads_pk").val()==""){
        sweetMensagem('warning',"Por favor, Informe o Posto de serviço.");
        return false;
    }
    ...
    utilsJS.jqueryConfirm('Ponto', 'Deseja bater o ponto de '+tipo_ponto+' ?', function () {
        iniciarEnvioRegistroPonto();
        formdataBaterPonto = new FormData();
...
$(document).ready(function()
    {
        try {
            ...
            $(".mask-cpf-cadastro").mask("000.000.000-00", { reverse: true });
        
            $(".mask-cpf-cadastro").on("input", function () {
                var el = $(this);
                setTimeout(function () {
            ...
            // Mostra o erro no celular
            alert("Erro ao iniciar scripts: " + e.message);
        
            // Também loga no console (para PC)
            console.error("Erro ao iniciar scripts:", e);
        }
