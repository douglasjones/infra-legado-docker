    ...
    if (arrCarregar.status == true) {
        if(arrCarregar.data.length>0){
            ...
            for(i=0;i<arrCarregar.data.length;i++){
                ...
                var validadoFolha = parseInt(arrCarregar.data[i]['pontos_dia'][0]['ic_status'] || 0, 10) === 1;
...
function fcSalvarValidadoReloginho(index){
    ...
    utilsJS.jqueryConfirm('Validar?', 'Ao validar, você está de acordo com as informações apresentadas nesta data. ', function () {
            ...
            if (arrEnviar && arrEnviar.status == true){
                ...
                utilsJS.toastNotify(false, (arrEnviar && arrEnviar.message) ? arrEnviar.message : 'Falhou a requisição para salvar o registro');
            }
