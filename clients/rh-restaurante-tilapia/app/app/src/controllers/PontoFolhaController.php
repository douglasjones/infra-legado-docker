...
final class PontoFolhaController extends BaseController {
    ...
    public function receptivoPrint(Request $request, Response $response, $args){
        try{
            ...
            $this->view->render($response, 'ponto_folha/ponto_folha_print_form.twig',array('pk'=>$pk, 'leads_pk'=>$leads_pk, 'colaborador_pk'=>$colaborador_pk, 'relatorio_banco_horas'=>$relatorio_banco_horas));
        } catch (Throwable $th) {
            return $response->withJson((object)[
                'error' => $th->getMessage()
            ], 500, []);
    ...
    public function listarConsultaPontoColaborador(Request $request, Response $response, $args) {
        ...
        try{
            ...
            if(isset($_SESSION['session_user']['par10']) && $_SESSION['session_user']['par10']==11 ){
                if($leads_pk==1588 || $leads_pk==1529){
                    json::run(true,[],"Você não tem permissão para acessar ! ");
                    die();
                }
            }
            json::run(true, is_array($retorno) ? $retorno : [], "");
            return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
        } catch (Throwable $th) {
            return $response->withJson((object)[
                'error' => $th->getMessage()
