...
class PontoFolha {
            ...
            for($i=0;$i<count($arrDadosRegistros);$i++){
                ...
                if($arrDadosRegistros[$i]['pk']  == ""){
                    ...
                    $retorno->data = $pk;
                        ...
                        $fieldsUpdate['hr_termino_intervalo'] = $arrDadosRegistros[$i]['hr_fim_intervalo'];
                        $fieldsUpdate['hr_termino_expediente'] = $arrDadosRegistros[$i]['hr_fim_expediente'];
    ...
    public function verificarFolhaColaborador($dt_periodo_ini,$dt_periodo_fim,$colaborador_pk,$ponto_folha_pk){
        ...
        $sql="";
        $sql.=" select pfc.pk, pfc.ic_status from ponto_folha_colaborador pfc
    ...
    public function listarDadosEscala($dt_periodo_ini, $dt_periodo_fim, $colaboradores_pk, $agenda_colaborador_padrao_pk){
        ...
        $sql="";
        $sql.=" SELECT a.tipos_escalas_pk tipo_escala,";
        $sql.="   te.ds_tipo_escala,";
        $sql.="   a.turnos_pk,";
        $sql.="   NULL hr_jornada_trabalho_intervalo,";
        $sql.="   a.pk,";
        ...
        if(count($query)>0){
            ...
            */
            ...
            $result[] = array(
                ...
                "hr_jornada_trabalho_intervalo"=>$query[0]['hr_jornada_trabalho_intervalo'],
    ...
    public function listarDiasEscala($agenda_colaborador_padrao_pk,$dt_periodo_ini,$dt_periodo_fim){
        ...
        $sql.="       dt_escala,";
        $sql.="       tipos_escalas_pk tipo_escala_pk,";
        $sql.="      CASE 
    ...
    public function listarDadosRegistrosFolha($ponto_folha_pk, $colaborador_pk ,$dt_periodo_ini,$dt_periodo_fim){
        ...
        $sql.="       ,tipos_ponto_pk tipo_ponto_pk";
        $sql.="       ,ic_status";
    ...
    public function getVerificado($dt_escala, $colaboradores_pk,$agenda_colaborador_padrao_pk){
        try {
            ...
            $sql.= "  select v.pk, v.ic_verificado 
                      ...
                      and a.pk = ".$agenda_colaborador_padrao_pk."
    ...
    public function listarDadosApontamento($dt_escala, $colaboradores_pk, $agenda_colaborador_padrao_pk,$ic_historico) {
        ...
        // Listar os apontamentos por tipo
        ...
        if(count($arrApontamentoDisciplina) != 0){
            $arrApontamento = array_merge($arrApontamento, $arrApontamentoDisciplina);
        }
    
        // Ordenar o array pelo campo 'dt_registro' em ordem crescente
        usort($arrApontamento, function($a, $b) {
            return strtotime($a['dt_registro']) - strtotime($b['dt_registro']);
        });
    
        // Pega o tipo_apontamento_pk do registro mais recente
        if (count($arrApontamento) > 0) {
            $tipo_apontamento_pk = $arrApontamento[count($arrApontamento)-1]['tipo_apontamento_dados_pk'];
            $apontamento_pk = $arrApontamento[count($arrApontamento)-1]['apontamento_pk'];
        }
    ...
    public function listarApontamentoPonto($dt_escala, $colaboradores_pk,$ic_historico){
        $sql = "";
        $sql.= "  SELECT 'Ativo' ds_status ,(1) AS tipo_apontamento_dados_pk,'' hr_excedentes,'' hr_faltantes,'' hr_trabalhadas, ap.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,ap.dt_ponto, ap.hr_ponto, ap.tipo_ponto_pk, ap.ds_obs_ponto";
    ...
    public function listarApontamentoFalta($dt_escala, $colaboradores_pk,$ic_historico){
        $sql = "";
        $sql.= "  SELECT '' hr_ini_abono,'' hr_ini_intervalo_abono,'' hr_termino_intervalo_abono,'' hr_termino_abono,'Ativo' ds_status , (2) AS tipo_apontamento_dados_pk, af.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,af.motivo_falta_pk, af.dt_falta, af.ds_obs_falta";
    ...
    public function listarApontamentoFolga($dt_escala, $colaboradores_pk,$ic_historico){
        $sql = "";
        $sql.= "  SELECT 'Ativo' ds_status , (3) AS tipo_apontamento_dados_pk, af.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,af.motivo_folga_pk, af.dt_folga, af.ds_obs_folga, af.apontamento_falta_pk,'' nome_feriado";
    ...
    public function listarApontamentoDisciplina($dt_escala, $colaboradores_pk,$ic_historico){
        $sql = "";
        $sql.= "  SELECT 'Ativo' ds_status , (8) AS tipo_apontamento_dados_pk, af.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk, af.dt_disciplina";
    ...
    public function listarApontamentoAfastamento($dt_escala, $colaboradores_pk,$ic_historico){
        ...
        $sql = "";
        $sql.= "  SELECT 'Ativo' ds_status ,  (5) AS tipo_apontamento_dados_pk, aa.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,aa.motivo_afastamento_pk, aa.dt_ini_afastamento, aa.dt_fim_afastamento, aa.ds_obs_afastamento";
    ...
    public function listarApontamentoFerias($dt_escala, $colaboradores_pk,$ic_historico){
        ...
        $sql = "";
        $sql.= "  SELECT 'Ativo' ds_status , (6) AS tipo_apontamento_dados_pk, af.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,af.dt_ini_ferias, af.dt_fim_ferias, af.ds_obs_ferias";
    ...
    public function listarModalPonto($colaborador_pk, $dt_escala){
        try{
            ...
            if (empty($dadosEscala) || !isset($dadosEscala[0])) {
            ...
            $hora ="";
            $arrDadosApontamento = [];
            if (isset($arrApontamento[0]['arrApontamento']) && is_array($arrApontamento[0]['arrApontamento'])) {
                $arrDadosApontamento = $arrApontamento[0]['arrApontamento'];
            }
                                ...
                                case 2:
                                    ...
                                    if($tipo_ponto_pk==2){
                                        ...
                                        $horas_trabalhadas = "";
                                            ...
                                            $ponto_ini_intervalo = "";
                                            $ponto_term_intervalo = "";
                                            if(!empty($arrDadosApontamento[$a]['hr_ini_abono'])){
                                                $ponto_ini_expediente = $arrDadosApontamento[$a]['hr_ini_abono'];
                                            }
                                            if(!empty($arrDadosApontamento[$a]['hr_ini_intervalo_abono'])){
                                                $ponto_ini_intervalo = $arrDadosApontamento[$a]['hr_ini_intervalo_abono'];
                                            }
                                            if(!empty($arrDadosApontamento[$a]['hr_termino_intervalo_abono'])){
                                                $ponto_term_intervalo = $arrDadosApontamento[$a]['hr_termino_intervalo_abono'];
                                            }
                                            if(!empty($arrDadosApontamento[$a]['hr_termino_abono'])){
                                                $ponto_term_expediente = $arrDadosApontamento[$a]['hr_termino_abono'];
                                            }
                                    }else if($tipo_ponto_pk==16){
                                        $obs = "Atestado";
                                        $ponto_ini_expediente = "";
    ...
    public function listarConsultaPontoColaborador($leads_pk, $colaboradores_pk, $dt_periodo_ini,$dt_periodo_fim,$agenda_colaborador_padrao_pk){              
        try{
            ...
            $intervalo_diario = "";
            $expediente = "";
            $arrPontoFolha = $this-> getPontoFolhaByColaboradorPeriodoLead($leads_pk, $colaboradores_pk, $dt_periodo_ini,$dt_periodo_fim);
            ...
            for($i=0; $i<count($diasEscala); $i++){  
                                ...
                                switch($tipo){
                                    ...
                                    case 2:
                                        ...
                                        if($tipo_ponto_pk==2){
                                            ...
                                            $situacao = "Abonada Horas";
                                            $tipo_ponto_pk = 1;
                                            if($arrDadosApontamento[$a]['hr_ini_abono']!=null){
                                                $ic_apontamento_ini = 1;
                                                $ponto_ini_expediente = $arrDadosApontamento[$a]['hr_ini_abono'];
                                            }                                          
                                            if($arrDadosApontamento[$a]['hr_ini_intervalo_abono']!=null){
                                                $ic_apontamento_ini_int = 3;
                                               
                                                $ponto_ini_intervalo = $arrDadosApontamento[$a]['hr_ini_intervalo_abono'];
                                            }
                                            if($arrDadosApontamento[$a]['hr_termino_intervalo_abono']!=null){
                                                $ic_apontamento_fim_int = 4;
                                                $ponto_term_intervalo = $arrDadosApontamento[$a]['hr_termino_intervalo_abono'];
                                            }
                                            if($arrDadosApontamento[$a]['hr_termino_abono']!=null){
                                                $ic_apontamento_ter = 2;
                                                $ponto_term_expediente = $arrDadosApontamento[$a]['hr_termino_abono'];
                                            }
                ...
                $registroFolhaDia = $this->getRegistroFolhaCompletoPorData(
                    isset($arrPontoFolha['pk']) ? $arrPontoFolha['pk'] : "",
                    $colaboradores_pk,
                    $dt_escala
                );

                if(!empty($registroFolhaDia)){
                    $ponto_ini_expediente = $registroFolhaDia['hr_ini_expediente'] ?? $ponto_ini_expediente;
                    $ponto_ini_intervalo = $registroFolhaDia['hr_ini_intervalo'] ?? $ponto_ini_intervalo;
                    $ponto_term_intervalo = $registroFolhaDia['hr_fim_intervalo'] ?? $ponto_term_intervalo;
                    $ponto_term_expediente = $registroFolhaDia['hr_fim_expediente'] ?? $ponto_term_expediente;
                    $hr_excedentes = $registroFolhaDia['hr_excedentes'] ?? $hr_excedentes;
                    $hr_faltante = $registroFolhaDia['hr_faltantes'] ?? $hr_faltante;
                    $horas_trabalhadas = $registroFolhaDia['hr_trabalhadas'] ?? $horas_trabalhadas;

                    if(!empty($registroFolhaDia['tipos_ponto_pk'])){
                        $tipo_ponto_pk = $registroFolhaDia['tipos_ponto_pk'];
                        $situacao = $this->descreverSituacaoTipoPonto($tipo_ponto_pk, $situacao);
                    }
                }
                ...
                $arrPontos[] = array(
                    ...
                    "apontamento_pk" => $apontamento_pk,
                    "tipo_ponto_pk" => $tipo_ponto_pk,
                    "ic_status" => $registroFolhaDia['ic_status'] ?? 0
                );
                ...
                $arrDias[] = array(
                    ...
                    "ds_turno" => $ds_turno,
                    "turnos_pk" => $turnos_pk,
                    "ponto_folha_pk" => $arrPontoFolha['pk'],
                    "ic_status_ponto_folha_pk" => $arrPontoFolha['ic_status'],
    ...
    public function verificarPontoEscalaNormal($dt_escala,$colaboradores_pk) {
        ...
        $sql.=' where p.colaborador_pk ='.$colaboradores_pk;
        $sql.=' AND (
            (p.dt_hora_ponto BETWEEN "'.$dt_escala.' 05:00:00" and  "'.$dt_escala.' 14:30:00" AND p.tipos_ponto_pk = 1)
            OR
            (p.dt_hora_ponto BETWEEN "'.$dt_escala.' 11:00:00" and  "'.$dt_escala.' 23:59:59" AND p.tipos_ponto_pk != 1)
        )';
        $sql.=" group by p.tipos_ponto_pk";
        ...
        // Verifica se existe pelo menos um registro com tipo_ponto_pk = 1
    ...
    public function verificarPontoEscalaNoturna($dt_escala,$colaboradores_pk) {
        ...
        $sql.=' where p.colaborador_pk ='.$colaboradores_pk;
        $sql.=' AND (
            (p.dt_hora_ponto BETWEEN "'.$dt_escala.' 14:30:00" and  "'.$dt_escala_modified.' 02:00:00" AND p.tipos_ponto_pk = 1)
            OR
            (p.dt_hora_ponto BETWEEN "'.$dt_escala_modified.' 00:00:00" and  "'.$dt_escala_modified.' 10:30:00" AND p.tipos_ponto_pk != 1)
        )';
        $sql.=" group by p.tipos_ponto_pk";
        $sql.=" order by p.dt_hora_ponto desc";
    ...
    public function getPontoFolhaPk($leads_pk,$colaborador_pk,$ic_mes,$ic_ano){
        try{
            ...
            if(count($query)>0){
                ...
                $fields = array();
                $fields['contas_pk'] = $dados->data[0]['contas_pk'] ?? ($_SESSION['session_user']['contas_pk'] ?? null);
    ...
    public function getRegistroFolhaPorData($ponto_folha_pk, $colaborador_pk, $dt_hora_ponto){
        if(empty($ponto_folha_pk) || empty($colaborador_pk) || empty($dt_hora_ponto)){
            return [];
        }

        $sql ="";
        $sql.=" SELECT pk, ic_status";
        $sql.=" FROM ponto_folha_registros";
        $sql.=" WHERE ponto_folha_pk = ".$ponto_folha_pk;
        $sql.="   AND colaborador_pk = ".$colaborador_pk;
        $sql.="   AND DATE(dt_hora_ponto) = '".$dt_hora_ponto."'";
    ...
    public function getRegistroFolhaCompletoPorData($ponto_folha_pk, $colaborador_pk, $dt_hora_ponto){
        if(empty($ponto_folha_pk) || empty($colaborador_pk) || empty($dt_hora_ponto)){
            return [];
        }
        ...
        $sql ="";
        $sql.=" SELECT pk,";
        $sql.="        TIME_FORMAT(hr_ini_expediente, '%H:%i') hr_ini_expediente,";
        $sql.="        TIME_FORMAT(hr_ini_intervalo, '%H:%i') hr_ini_intervalo,";
        $sql.="        TIME_FORMAT(hr_termino_intervalo, '%H:%i') hr_fim_intervalo,";
        $sql.="        TIME_FORMAT(hr_termino_expediente, '%H:%i') hr_fim_expediente,";
        $sql.="        TIME_FORMAT(hr_trabalhadas, '%H:%i') hr_trabalhadas,";
        $sql.="        TIME_FORMAT(hr_excedente, '%H:%i') hr_excedentes,";
        $sql.="        TIME_FORMAT(hr_faltantes, '%H:%i') hr_faltantes,";
        $sql.="        tipos_ponto_pk,";
        $sql.="        ic_status,";
        $sql.="        obs";
        $sql.=" FROM ponto_folha_registros";
    ...
    public function descreverSituacaoTipoPonto($tipo_ponto_pk, $situacaoAtual = ""){
        switch ((int)$tipo_ponto_pk) {
            case 1:
                return "Ponto/Expediente";
            case 2:
                return "Falta";
            case 3:
                return "Folga";
            ...
            case 8:
                return "Disciplina";
            case 11:
                return "Abonada Horas";
            case 16:
                return "Atestado";
            case 17:
                return "Advertencia";
            case 18:
                return "Declaração da defesa civil";
            case 19:
                return "Demissão";
            case 20:
                return "Folga compensatória";
            case 21:
                return "Folga de feriado";
            case 22:
                return "Justa causa";
            case 23:
                return "Recisão indireta";
            case 24:
                return "Suspensão";
            case 25:
                return "Troca Folga";
            case 26:
                return "Folga trabalhada";
            case 27:
                return "Escala Errada";
            case 28:
                return "Apoio Operacional";
            case 29:
                return "Atestado por acompanhar filho ate 5 anos";
            case 30:
                return "Atestado por serviço Justiça Eleitoral";
            case 31:
                return "Doação de sangue";
            case 32:
                return "Atraso";
            case 33:
                return "Declaração de horas abonar";
            case 34:
    ...
    public function gerarFolhaPontoByRelogio($arrDados){
        ...
        try{
            $arrDados = is_array($arrDados) ? $arrDados : json_decode((string)$arrDados, true);
            if (!is_array($arrDados) || empty($arrDados) || !isset($arrDados[0])) {
                $retorno->message = 'Dados do reloginho inválidos';
                return $retorno;
            }
            $usuarioSessaoPk = isset($_SESSION['session_user']['par1']) ? $_SESSION['session_user']['par1'] : 0;
            ...
            if(count($countColaborador)==0){
                ...
                $fields["dt_ult_atualizacao"] = "sysdate()";
                $fields["usuario_ult_atualizacao_pk"] = $usuarioSessaoPk;
                $fields["dt_cadastro"] = "sysdate()";
                $fields["usuario_cadastro_pk"] = $usuarioSessaoPk;

                // Insere o colaborador na tabela `ponto_folha_colaborador`
                Util::execInsert("ponto_folha_colaborador", $fields, $this->pdo);

                
            }
            else if($countColaborador[0]['ic_status']==1){
                $retorno->status = true;
                $retorno->message = 'Dados atualizado com sucesso';
                $retorno->data = [];
                return $retorno;
            ...
            for($i=0;$i<count($arrDados);$i++){
                $dtRegistro = !empty($arrDados[$i]['dt_hora_ponto']) ? Util::DataYMD($arrDados[$i]['dt_hora_ponto']) : "";
                if(empty($dtRegistro)){
                    continue;
                }

                $registroExistente = $this->getRegistroFolhaPorData(
                    $ponto_folha_pk,
                    $arrDados[$i]['colaborador_pk'] ?? "",
                    $dtRegistro
                );

                if(isset($registroExistente['ic_status']) && (int)$registroExistente['ic_status'] === 1){
                    continue;
                }

                $fields = array();
                $fields['dt_hora_ponto'] = $dtRegistro;
                $fields['colaborador_pk'] = $arrDados[$i]['colaborador_pk'] ?? "";
                $fields['ponto_folha_pk'] = $ponto_folha_pk;
                $fields['ic_status'] = (isset($arrDados[$i]['ic_status']) && $arrDados[$i]['ic_status']==0) ? 1 : 0;
                $fields['hr_ini_expediente'] = !empty($arrDados[$i]['hr_ini_expediente']) ? $arrDados[$i]['hr_ini_expediente'] : "";
                $fields['hr_ini_intervalo'] = !empty($arrDados[$i]['hr_ini_intervalo']) ? $arrDados[$i]['hr_ini_intervalo'] : "";
                $fields['hr_termino_intervalo'] = !empty($arrDados[$i]['hr_fim_intervalo']) ? $arrDados[$i]['hr_fim_intervalo'] : "";
                $fields['hr_termino_expediente'] = !empty($arrDados[$i]['hr_fim_expediente']) ? $arrDados[$i]['hr_fim_expediente'] : "";
                ...
                $fields['tipos_ponto_pk'] = $arrDados[$i]['tipo_ponto_pk'] ?? "";
                $fields["dt_ult_atualizacao"] = "sysdate()";
                $fields["usuario_ult_atualizacao_pk"] = $usuarioSessaoPk;

                if(isset($registroExistente['pk']) && !empty($registroExistente['pk'])){
                    Util::execUpdate("ponto_folha_registros", $fields, "pk = ".$registroExistente['pk'], $this->pdo);
                    $registrosAfetados[] = $registroExistente['pk'];
                }
                else{
                    $fields["dt_cadastro"] = "sysdate()";
                    $fields["usuario_cadastro_pk"] = $usuarioSessaoPk;
                    $registrosAfetados[] = Util::execInsert("ponto_folha_registros", $fields, $this->pdo);
                }
            }

            $retorno->status = true;
            $retorno->message = 'Dados atualizado com sucesso';
            $retorno->data = $registrosAfetados;
