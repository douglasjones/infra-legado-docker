...
class Ponto {
    ...
    private $margemFimTurnoNoturnoSegundos = 14400;
    ...
    {
        $sql = "";
        $sql .= "SELECT hr_inicio_expediente, hr_termino_expediente";
        $sql .= "  FROM agenda_colaborador_padrao";
        $sql .= " WHERE dt_cancelamento IS NULL";

        if ($agenda_colaborador_padrao_pk !== "") {
            $sql .= "   AND pk = " . (int)$agenda_colaborador_padrao_pk;
        } else {
            $sql .= "   AND colaboradores_pk = " . (int)$colaborador_pk;
        }

        $sql .= " ORDER BY pk DESC LIMIT 1";

        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        $escala = $stmt->fetch(\PDO::FETCH_ASSOC) ?: [];

        $hr_inicio_expediente = $this->normalizarHorarioEscala($escala['hr_inicio_expediente'] ?? "", '16:00:00');
        $hr_termino_expediente = $this->normalizarHorarioEscala($escala['hr_termino_expediente'] ?? "", '10:00:00');
    ...
    private function formatarImagemPonto($imgPonto)
    {
        if ($imgPonto === null || $imgPonto === "") {
            return "";
        }

        if (strpos($imgPonto, 'data:image') === 0) {
            return '<img width="80" height="80" src="' . $imgPonto . '">';
        }

        return '<img width="80" height="80" src="data:image/png;base64,' . base64_encode($imgPonto) . '">';
    }
    ...
    public function pegarPontoNormal($dt_ini, $dt_fim, $colaborador_pk, $leads_pk)
    {
        ...
        $sql .= "       pt.tipos_ponto_pk tipo_ponto_pk,";
        ...
        $sql .= "  FROM ponto pt";
        $sql .= "  LEFT JOIN colaboradores col ON pt.colaborador_pk = col.pk";
        ...
        $sql .= " WHERE pt.dt_hora_ponto BETWEEN '" . $dt_escala . " 00:00:00' AND '" . $dt_escala . " 23:59:59'";
        $sql .= "   AND pt.colaborador_pk = " . (int)$colaborador_pk;

        if ($leads_pk !== "") {
            $sql .= "   AND pt.leads_pk = " . (int)$leads_pk;
        }
    ...
    public function pegarPontoNoturno($dt_ini, $dt_fim, $colaborador_pk, $leads_pk, $agenda_colaborador_padrao_pk = "")
    {
        ...
        $sql .= "  LEFT JOIN produtos_servicos ps ON cps.produtos_servicos_pk = ps.pk";
        $sql .= "  LEFT JOIN leads l ON pt.leads_pk = l.pk";
        $sql .= " WHERE pt.dt_hora_ponto BETWEEN '" . $janela['inicio'] . "' AND '" . $janela['fim'] . "'";
        $sql .= "   AND pt.colaborador_pk = " . (int)$colaborador_pk;
    ...
    public function reloginhoHistoricoPonto($leads_pk, $colaborador_pk, $dt_ini, $dt_fim, $agenda_colaborador_padrao_pk)
    {
        ...
        try {
            ...
            foreach ($query as $row) {
                ...
                switch ((int)$row['tipo_ponto_pk']) {
                    case 1:
                        $ds_legenda = "Inicio Expediente";
                        break;
                    case 2:
                        $ds_legenda = "Fim Expediente";
                        break;
                    case 3:
                        $ds_legenda = "Saida p/ Intervalo";
                        break;
                    case 4:
                        $ds_legenda = "Retorno do Intervalo";
                        break;
                }
                ...
                $mysql_data[] = [
                    ...
                    "ds_lead" => $row["ds_lead"] ?? "",
                    "ds_pin" => $row["ds_pin"] ?? "",
                    "ds_colaborador" => $row["ds_colaborador"] ?? "",
                    "colaborador_pk" => $colaborador_pk,
                    "ds_produto_servico" => $row["ds_produto_servico"] ?? "",
                    "dt_rh_entratada" => $row["dt_rh_entratada"] ?? "",
                    "ds_localizacao" => $row["ds_localizacao"] ?? "",
                    "ds_imagem_entrada" => $row["ds_imagem_entrada"] ?? "",
                    "ds_legenda" => $ds_legenda,
                    "ds_registro_ponto" => $row["hr_entrada"] ?? "",
                    "img_ponto" => $this->formatarImagemPonto($row["img_ponto"] ?? ""),
                    "ds_distancia_ponto" => $row["ds_distancia_ponto"] ?? "",
                ];
            }
