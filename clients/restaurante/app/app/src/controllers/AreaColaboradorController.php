...
final class AreaColaboradorController extends BaseController {
    ...
    public function pegarInfoColaborador(Request $request, Response $response, $args){
        try{
            ...
            $id_colaborador = isset($data['id_colaborador'])? trim($data['id_colaborador']): "";

            if(strpos($id_colaborador, '-') !== false){
                $arrPinColaborador = explode('-', $id_colaborador);
                if($id_empresa == ""){
                    $id_empresa = trim($arrPinColaborador[0]);
                }
                $id_colaborador = trim(end($arrPinColaborador));
            }
    ...
    public function salvarPonto(Request $request, Response $response, $args){
        try{
            ...
            if (empty($retornoAgenda->data) || empty($retornoAgenda->data[0]['pk'])) {
                Json::run(false, [], "Escala/Posto de trabalho nao localizado para este colaborador.");
            ...
            if (!is_dir($targetDirectory) && !mkdir($targetDirectory, 0775, true)) {
                Json::run(false, [], "Diretorio de fotos do ponto nao existe ou nao pode ser criado.");
                return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
            }

            if (!is_writable($targetDirectory)) {
                Json::run(false, [], "Diretorio de fotos do ponto sem permissao de escrita.");
                return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
            }
