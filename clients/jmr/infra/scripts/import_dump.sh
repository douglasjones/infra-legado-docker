...
CONTAINER_NAME="${CONTAINER_NAME:-jmr-mysql}"
DB_NAME="${DB_NAME:-gepros1com_jmr}"
DB_USER="${DB_USER:-root}"
DB_PASSWORD="${DB_PASSWORD:-SENHA}"
DB_CHARSET="${DB_CHARSET:-latin1}"
DB_COLLATION="${DB_COLLATION:-latin1_swedish_ci}"
DUMP_SQL="${DUMP_SQL:-${BACKUP_DIR}/gepros1com_jmr.sql}"
DUMP_GZ="${DUMP_GZ:-${BACKUP_DIR}/gepros1com_jmr.sql.gz}"
CONTAINER_TMP_PATH="${CONTAINER_TMP_PATH:-/tmp/jmr_dump.sql}"
...
apply_compat_migrations() {
  log "Aplicando compatibilidade do app brasil-servis para base JMR"
  docker exec "${CONTAINER_NAME}" mysql -u"${DB_USER}" -p"${DB_PASSWORD}" "${DB_NAME}" -e "
    SET @column_exists := (
      SELECT COUNT(*)
       ...
       WHERE table_schema = '${DB_NAME}'
         ...
         AND column_name = 'ds_dominio'
    );
    SET @ddl := IF(
      @column_exists = 0,
      'ALTER TABLE contas ADD COLUMN ds_dominio varchar(255) DEFAULT NULL',
      'SELECT 1'
    );
    ...
    SET @ddl := IF(
      @column_exists = 0,
      'ALTER TABLE leads ADD COLUMN dia_faturamento int(11) DEFAULT NULL',
      'SELECT 1'
    );
    PREPARE stmt FROM @ddl;
    EXECUTE stmt;
    ...
    SET @ddl := IF(
      @column_exists = 0,
      'ALTER TABLE apontamento_folga ADD COLUMN feriado_pk int(11) DEFAULT NULL',
      'SELECT 1'
    );
    PREPARE stmt FROM @ddl;
    EXECUTE stmt;
