...
function fcSalvarContrato() {
    ...
    var v_vl_mao_oabra = 0;
    if ($('#vl_total_mao_obra').html() != '') {
        v_vl_mao_oabra = toNumberOrZero(moeda2float($('#vl_total_mao_obra').html()));
    }
    ...
    var objParametros = {
        ...
        "vl_contrato": toNumberOrZero(moeda2float($("#vl_contrato").val()))
    };
    ...
    if (arrEnviar && arrEnviar.status == true) {
        ...
        utilsJS.toastNotify(false, (arrEnviar && (arrEnviar.message || arrEnviar.result)) ? (arrEnviar.message || arrEnviar.result) : 'Falha ao salvar contrato.');
    }
    return true;
}
    ...
    for (i = 0; i < (cboProdutosPk.length); i++) {
        try {
            ...
            if (vl_mao_obra.get(i).value != '') {
                l_vl_mao_obra = toNumberOrZero(moeda2float(vl_mao_obra.get(i).value))
            }
            arrDados[i] = [
                contratos_itens_pk_2.get(i).value,
                cboProdutosPk.get(i).value,
                n_qtde_contratos_itens.get(i).value,
                toNumberOrZero(moeda2float(vl_unit.get(i).value)),
                toNumberOrZero(moeda2float(vl_total.get(i).value)),
