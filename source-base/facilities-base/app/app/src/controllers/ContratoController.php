...
final class ContratoController extends BaseController {
    ...
    public function salvar(Request $request, Response $response, $args) {
        try{
            ...
            $contrato =[
                ...
                "vl_total_mao_obra"=>$this->normalizarValorDecimal($vl_total_mao_obra),
                "ds_identificacao_area"=>$ds_identificacao_area,
                "vl_contrato"=>$this->normalizarValorDecimal($vl_contrato)
            ];

            $retorno = (new Contrato($this->pdo))->salvar($contrato);

            if ($retorno->status !== true) {
                Json::run(false, [], $retorno->message ?: 'Falha ao salvar contrato.');
            ...
            if(count($arrContratoItens) > 0){
                ...
                for($i = 0; $i < count($arrContratoItens); $i++){
                    ...
                    $valor_unitario = $this->normalizarValorDecimal($arrContratoItens[$i]["vl_unit"]);
                    $valor_total = $this->normalizarValorDecimal($arrContratoItens[$i]['n_qtde']) * $valor_unitario;
                    $vl_mao_obra = $this->normalizarValorDecimal($arrContratoItens[$i]["vl_mao_obra"]);

                    (new Contrato($this->pdo))->adicionarContratoItens($arrContratoItens[$i]['contratos_itens_pk'],$contratos_pk,$arrContratoItens[$i]['n_qtde_dias_semana'], $arrContratoItens[$i]['n_qtde'], $valor_unitario, $valor_total, $arrContratoItens[$i]["produtos_servicos_pk"],$arrContratoItens[$i]["periodo"],$vl_mao_obra);
                }
            }
    ...
    private function normalizarValorDecimal($valor) {
