    ...
    public function download(Request $request, Response $response, $args) {
        try{
            ...
            if($pk_doc_bd>0){
                ...
                $documentoBlob = $retorno->data[0]["docsData"];
                $documentoType = $retorno->data[0]["docsType"];
                $fileName = $retorno->data[0]["ds_nome_original"] ?: $retorno->data[0]["ds_documento"] ?: $ds_documento;
                $fileName = basename(str_replace(["\r", "\n", '"'], "", $fileName));
                $fileName = $fileName ?: "documento";
                
                if ($documentoBlob) {
                    if (Util::isBase64($documentoBlob)) {
                        $documentoBlob = base64_decode($documentoBlob);
                    }
                    else{
                        $documentoBlob = $documentoBlob;
                    }

                    $ext = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
                    $disposition = $ext == "pdf" ? "inline" : "attachment";
                    $documentoType = $documentoType ?: "application/octet-stream";

                    header("Content-Type: " . $documentoType);
                    header("Content-Length: " . strlen($documentoBlob));
