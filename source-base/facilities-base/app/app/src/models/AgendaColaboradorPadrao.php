...
class AgendaColaboradorPadrao {
    ...
    private function buscarAgendaPorPk($pk){
        if(empty($pk)){
            return [];
        }

        $stmt = $this->pdo->prepare("SELECT n_qtde_dias_semana, tipo_escala FROM agenda_colaborador_padrao WHERE pk = :pk");
    ...
    public function salvar($agenda_colaborador_padrao){
        try{
            ...
            $agendaAtual = $this->buscarAgendaPorPk($agenda_colaborador_padrao['pk']);
            $tipoEscala = $agenda_colaborador_padrao['tipo_escala'];
            if(($tipoEscala === "" || $tipoEscala === null) && isset($agendaAtual['tipo_escala'])){
                $tipoEscala = $agendaAtual['tipo_escala'];
            }

            $fields['tipo_escala'] = $tipoEscala;
            ...
            $diasEscalaServico = $agenda_colaborador_padrao['n_qtde_dias_semana'];
            if($diasEscalaServico === "" || $diasEscalaServico === null){
                $diasEscalaServico = $agenda_colaborador_padrao['dias_escala_servico'];
            }
            if(($diasEscalaServico === "" || $diasEscalaServico === null) && isset($agendaAtual['n_qtde_dias_semana'])){
                $diasEscalaServico = $agendaAtual['n_qtde_dias_semana'];
            }
            if($diasEscalaServico === "" || $diasEscalaServico === null){
                $retorno->status = false;
                $retorno->message = 'Tipo da escala não informado.';
                return $retorno;
            }

            $fields['n_qtde_dias_semana'] = $diasEscalaServico;
    ...
    $dias_escala_servico){
        ...
        try{
            ...
            if(($ds_escala == "" || $ds_escala === null) && isset($queryEscala[0]['n_qtde_dias_semana'])){
                $ds_escala = $queryEscala[0]['n_qtde_dias_semana'];
            }
            if(($dias_escala_servico == "" || $dias_escala_servico === null) && $ds_escala != ""){
                $dias_escala_servico = $ds_escala;
            }
            if($ds_escala == "" || $ds_escala === null){
                $retorno->status = false;
                $retorno->message = "Tipo da escala não informado.";
                return $retorno;
            }
            if(count($queryEscala) == 0){
                $retorno->status = false;
                $retorno->message = "Agenda da escala não encontrada para o período informado.";
                return $retorno;
            }
            ...
            for ($a = 0; $a < $qtdeDias; $a++) {
                ...
                if ($ds_escala == '12x36') {
                    ...
                    if ($fl_escala_alternada == 1) {


                        if($verificarEscala==1){
                            if ($diasemana[$diasemana_numero] == "Dom") {
                            ...
                            $verificarEscala = $ic_escala; 
                        }
                        else{
                            
                            // Aplica a lógica de folga dependendo do tipo de escala
                            if ($dias_escala_servico == '4x1') {
                                if($contadorEscala<4){
                                    $ic_escala = 1;
                                    $contadorEscala++;
                                    $contadorFolga = 0;
                            ...
                            } elseif ($dias_escala_servico == '4x2') {
                                //APLICAR DEPOIS
                            } elseif ($dias_escala_servico == '5x1') {
                                if($contadorEscala<5){
                            ...
                            } elseif ($dias_escala_servico == '5x2') {
                                //APLICAR DEPOIS
                            } elseif ($dias_escala_servico == '6x1') {
                                if($contadorEscala<6){
