        ...
        try{
            ...
            $sql.="SELECT l.ds_lead posto_trabalho,
                    ...
                    case a.tipo_apontamento_pk
                        WHEN 2 THEN 'Falta'
                        WHEN 11 THEN 'Abonada'
                        WHEN 16 THEN 'Atestado'
                        WHEN 18 THEN 'Declaração da defesa civil'
                        WHEN 28 THEN 'Apoio Operacional'
                        WHEN 29 THEN 'Atestado por acompanhar filho ate 5 anos'
                        WHEN 30 THEN 'Atestado por serviço Justiça Eleitoral'
                        WHEN 37 THEN 'Atestado de horas'
                        WHEN 31 THEN 'Doação de Sangue'
                        WHEN 32 THEN 'Atraso'
                        WHEN 33 THEN 'Declaração de horas abonar'
                        WHEN 34 THEN 'Sem Justificativa'
                        WHEN 35 THEN 'Reciclagem'
                        WHEN 36 THEN 'Audiência'
                    end ds_apontamento
                FROM agenda_colaborador_apontamento a
                INNER JOIN apontamento_falta af on af.agenda_colaborador_apontamento_pk = a.pk
                INNER JOIN (
                    SELECT ax.colaborador_pk, IFNULL(ax.leads_pk, 0) leads_pk, afx.dt_falta, MAX(ax.pk) pk
                      FROM agenda_colaborador_apontamento ax
                     INNER JOIN apontamento_falta afx ON afx.agenda_colaborador_apontamento_pk = ax.pk
                     WHERE afx.dt_falta BETWEEN '".$primeiroDiaMesAtual."' and '".$ultimoDiaMesAtual."'
                     GROUP BY ax.colaborador_pk, IFNULL(ax.leads_pk, 0), afx.dt_falta
                ) ult_apontamento ON ult_apontamento.pk = a.pk
                ...
                if($tipo_apontamento_pk!=""){
                    $arrTipoApontamento = array_filter(array_map('intval', explode(',', $tipo_apontamento_pk)));
                    if(count($arrTipoApontamento)>0){
                        $sql.=" AND a.tipo_apontamento_pk IN (".implode(',', $arrTipoApontamento).")";
                    }
                }
