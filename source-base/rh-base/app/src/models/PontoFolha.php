...
class PontoFolha {
    ...
    public function listarDadosEscala($dt_periodo_ini, $dt_periodo_fim, $colaboradores_pk, $agenda_colaborador_padrao_pk){
        ...
        $sql.="   a.turnos_pk,";
        $sql.="   a.hr_jornada_trabalho_intervalo,";
        $sql.="   a.pk,";
    ...
    public function listarApontamentoFalta($dt_escala, $colaboradores_pk,$ic_historico){
        $sql = "";
        $sql.= "  SELECT af.hr_ini_abono,af.hr_ini_intervalo_abono,af.hr_termino_intervalo_abono,af.hr_termino_abono,CASE acp.ic_status when 1 then 'Ativo' when 2 then 'Excluido' end ds_status , (2) AS tipo_apontamento_dados_pk, af.dt_cadastro dt_registro, date_format(acp.dt_cadastro,'%d/%m/%Y %H:%i:%s')dt_cadastro , u.ds_usuario,acp.pk apontamento_pk,acp.tipo_apontamento_pk,af.motivo_falta_pk, af.dt_falta, af.ds_obs_falta";
    ...
    public function getPontoFolhaPk($leads_pk,$colaborador_pk,$ic_mes,$ic_ano){
        try{
            ...
            if(count($query)>0){
                ...
                $fields = array();
                $fields['contas_pk'] = $dados->data[0]['empresas_pk'];
    ...
    public function gerarFolhaPontoByRelogio($arrDados){
        ...
        try{
            ...
            if(count($countColaborador)==0){
                ...
                Util::execDelete('ponto_folha_registros'," ponto_folha_pk = ".$ponto_folha_pk,$this->pdo);
                for($i=0;$i<count($arrDados);$i++){
                    ...
                    if($arrDados[$i]['hr_fim_intervalo']!=""){
                        $fields['hr_termino_intervalo'] = $arrDados[$i]['hr_fim_intervalo'];
                    }
                    else{
                        $fields['hr_termino_intervalo'] = "";
                    }
                    
                    if($arrDados[$i]['hr_termino_expediente']!=""){
                        $fields['hr_termino_expediente'] = $arrDados[$i]['hr_fim_expediente'];
                    }
                    else{
                        $fields['hr_termino_expediente'] = "";
                    }
                ...
                if($countColaborador[0]['ic_status']!=1){
