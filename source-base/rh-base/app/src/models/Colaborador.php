...
class Colaborador {
    ...
    public function salvar($colaborador){
        ...
        try{
            ...
            $this->salvarColaboradorServidor($colaborador);
            return $retorno;
        }
        catch(Throwable $e){
            print_r($e->getMessage());
    ...
    public function listarDataTable($pk,$grupos_leads_pk,$leads_pk,$ds_cpf,$cargo_pk,$ds_pin,$ic_status,$supervisores_pk){
        $retorno = new \StdClass; //Estrutura de retorno para controller
        $retorno->status = false; //Retorno setado status como false
        $retorno->data = []; //Retorno data setado como vazio
        try{
            ...
            $this->pdo->exec("SET SESSION sql_mode = REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', '')");
            $sql ="";
            $sql.="SELECT ";
            $sql.="                DISTINCT(c.pk),";
            $sql.="                c.ds_colaborador,";
            ...
            if($supervisores_pk != ""){
                $sql.=" and (l.supervisor_pk =".$supervisores_pk." or l.supervisor2_pk =".$supervisores_pk.")";
            }
            if($ic_status != ""){
    ...
    public function listarColaboradorFolha($empresas_pk, $leads_pk, $ic_escala){
        ...
        $sql.="        CASE c.ic_status WHEN 1 THEN 'Ativo' WHEN 2 THEN 'Desligado' END  ds_status_colaborador,";
    ...
    public function verificarColaboradorAtivoParaBaseWebPonto($colaborador_pk){
        ...
        $sql.="                left join contas ct on c.contas_pk = ct.pk";
     ...
     public function salvarColaboradorServidor($colaborador){
        $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
        // Obter o nome do host (ex: www.example.com)
        $host = $_SERVER['HTTP_HOST'];
        // Montar a URL completa
